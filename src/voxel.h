...
public:
	...
	VoxelArea(const v3size &min_edge, const v3size &max_edge):
		MinEdge(min_edge),
		MaxEdge(max_edge)
	{
		cacheExtent();
	...
	*/
	...
	{
		return MaxEdge - MinEdge == v3size(-1, -1, -1);
	}
	...
	VoxelArea operator+(const v3size &off) const
	{
	...
	*/
	void diff(const VoxelArea &a, std::list<VoxelArea> &result)
	{
		...
		*/
		...
		{
			v3size min(MinEdge.X, MinEdge.Y, a.MaxEdge.Z+1);
			v3size max(MaxEdge.X, MaxEdge.Y, MaxEdge.Z);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
		...
		{
			v3size min(MinEdge.X, MinEdge.Y, MinEdge.Z);
			v3size max(MaxEdge.X, MaxEdge.Y, a.MinEdge.Z-1);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
		...
		{
			v3size min(MinEdge.X, a.MaxEdge.Y+1, a.MinEdge.Z);
			v3size max(MaxEdge.X, MaxEdge.Y, a.MaxEdge.Z);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
		...
		{
			v3size min(MinEdge.X, MinEdge.Y, a.MinEdge.Z);
			v3size max(MaxEdge.X, a.MinEdge.Y-1, a.MaxEdge.Z);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
		...
		{
			v3size min(MinEdge.X, a.MinEdge.Y, a.MinEdge.Z);
			v3size max(a.MinEdge.X-1, a.MaxEdge.Y, a.MaxEdge.Z);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
		...
		{
			v3size min(a.MaxEdge.X+1, a.MinEdge.Y, a.MinEdge.Z);
			v3size max(MaxEdge.X, a.MaxEdge.Y, a.MaxEdge.Z);
			VoxelArea b(min, max);
			if(b.getVolume() != 0)
				result.push_back(b);
		}
	...
	*/
	 ...
	 */
	static void add_x(const v3size &extent, u32 &i, s16 a)
	{
		i += a;
	}
	 ...
	 */
	static void add_y(const v3size &extent, u32 &i, s16 a)
	{
		i += a * extent.X;
	}
	 ...
	 */
	static void add_z(const v3size &extent, u32 &i, s16 a)
	{
	 ...
	 */
	static void add_p(const v3size &extent, u32 &i, v3size a)
	{
	...
	*/
	...
	// Edges are inclusive
	v3size MinEdge = v3size(1,1,1);
	v3size MaxEdge;
private:
	void cacheExtent()
	{
		m_cache_extent = MaxEdge - MinEdge + v3size(1,1,1);
	}

	v3size m_cache_extent = v3size(0,0,0);
};
...
public:
	...
	*/
	MapNode getNode(const v3size &p)
	{
	...
	MapNode getNodeNoEx(const v3size &p)
	{
		VoxelArea voxel_area(p);
		addArea(voxel_area);
	...
	MapNode getNodeNoExNoEmerge(const v3size &p)
	{
	...
	MapNode & getNodeRefUnsafe(const v3size &p)
	{
		return m_data[m_area.index(p)];
	}

	const MapNode & getNodeRefUnsafeCheckFlags(const v3size &p)
	{
	...
	u8 & getFlagsRefUnsafe(const v3size &p)
	{
		return m_flags[m_area.index(p)];
	}

	bool exists(const v3size &p)
	{
		return m_area.contains(p) &&
			!(getFlagsRefUnsafe(p) & VOXELFLAG_NO_DATA);
	}

	void setNode(const v3size &p, const MapNode &n)
	{
		VoxelArea voxel_area(p);
		addArea(voxel_area);
	...
	void setNodeNoRef(const v3size &p, const MapNode &n)
	{
		setNode(p, n);
	}
	...
	*/
	bool setNodeNoEmerge(const v3size &p, MapNode n)
	{
		if(!m_area.contains(p))
			return false;
		m_data[m_area.index(p)] = n;
	...
	*/
	...
	/*
		...
		addInternalBox should not be used if getExtent() == v3size(0,0,0)
