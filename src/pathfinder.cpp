...
public:
	 ...
	 * @cost cost to set
	 */
	void      setCost(v3s32 dir, const PathCost &cost);
	...
	v3s32     sourcedir;                   /**< origin of movement for current cost   */
	v3s32     pos;                         /**< real position of node                 */
...
public:
	virtual PathGridnode &access(v3s32 p)=0;
	virtual ~GridNodeContainer() = default;

protected:
	Pathfinder *m_pathf;

	void initNode(v3s32 ipos, PathGridnode *p_node);
};
...
public:
	...
	ArrayGridNodeContainer(Pathfinder *pathf, v3s32 dimensions);
	virtual PathGridnode &access(v3s32 p);
...
public:
	...
	virtual PathGridnode &access(v3s32 p);
private:
	std::map<v3s32, PathGridnode> m_nodes;
};
...
public:
	 ...
	 */
	std::vector<v3s32> getPath(v3s32 source,
			v3s32 destination,
...
private:
	 ...
	 */
	PathGridnode &getIndexElement(v3s32 ipos);

	/**
	 * Get gridnode at a specific index position
	 * @return gridnode for index
	 */
	PathGridnode &getIdxElem(s32 x, s32 y, s32 z);
	 ...
	 */
	int           getXZManhattanDist(v3s32 pos);
	 ...
	 */
	PathCost     calcCost(v3s32 pos, v3s32 dir);
	 ...
	 */
	bool          updateAllCosts(v3s32 ipos, v3s32 srcdir, int current_cost, int level);
	 ...
	 * @return true/false path to destination has been found
	 */
	bool          updateCostHeuristic(v3s32 isource, v3s32 idestination);
	 ...
	 */
	bool          buildPath(std::vector<v3s32> &path, v3s32 ipos);
	 ...
	 * pos is returned
	 */
	v3s32         walkDownwards(v3s32 pos, unsigned int max_down);
	...
	v3s32 m_start;                /**< source position                          */
	v3s32 m_destination;          /**< destination position                     */

	core::aabbox3d<s32> m_limits; /**< position limits in real map coordinates  */
	 ...
	 */
	void printPath(std::vector<v3s32> path);
...
class PathfinderCompareHeuristic
{
	...
	public:
		...
		bool operator() (v3s32 pos1, v3s32 pos2) {
			v3s32 ipos1 = myPathfinder->getIndexPos(pos1);
			v3s32 ipos2 = myPathfinder->getIndexPos(pos2);
...
std::vector<v3s32> get_path(Map* map, const NodeDefManager *ndef,
		v3s32 source,
		v3s32 destination,
		unsigned int searchdistance,
		unsigned int max_jump,
...
/******************************************************************************/
PathCost PathGridnode::getCost(v3s32 dir)
{
	if (dir.X > 0) {
...
/******************************************************************************/
void PathGridnode::setCost(v3s32 dir, const PathCost &cost)
{
	if (dir.X > 0) {
...
void GridNodeContainer::initNode(v3s32 ipos, PathGridnode *p_node)
{
	const NodeDefManager *ndef = m_pathf->m_ndef;
	PathGridnode &elem = *p_node;

	v3s32 realpos = m_pathf->getRealPos(ipos);

	MapNode current = m_pathf->m_map->getNode(realpos);
	MapNode below   = m_pathf->m_map->getNode(realpos + v3s32(0, -1, 0));
	...
	if (m_pathf->m_prefetch) {
		elem.directions[DIR_XP] = m_pathf->calcCost(realpos, v3s32( 1, 0, 0));
		elem.directions[DIR_XM] = m_pathf->calcCost(realpos, v3s32(-1, 0, 0));
		elem.directions[DIR_ZP] = m_pathf->calcCost(realpos, v3s32( 0, 0, 1));
		elem.directions[DIR_ZM] = m_pathf->calcCost(realpos, v3s32( 0, 0,-1));
	}
}

ArrayGridNodeContainer::ArrayGridNodeContainer(Pathfinder *pathf, v3s32 dimensions) :
...
PathGridnode &ArrayGridNodeContainer::access(v3s32 p)
{
...
PathGridnode &MapGridNodeContainer::access(v3s32 p)
{
	std::map<v3s32, PathGridnode>::iterator it = m_nodes.find(p);
	if (it != m_nodes.end()) {
		return it->second;
	}
...
/******************************************************************************/
std::vector<v3s32> Pathfinder::getPath(v3s32 source,
							v3s32 destination,
							unsigned int searchdistance,
							unsigned int max_jump,
							unsigned int max_drop,
							PathAlgorithm algo)
{
	...
	v3s32 diff = m_limits.MaxEdge - m_limits.MinEdge;
	...
	//a walkable node.
	v3s32 true_source = v3s32(source);
	...
	v3s32 true_destination = v3s32(destination);
	destination = walkDownwards(destination, m_maxjump);

	//validate and mark start and end pos

	v3s32 StartIndex  = getIndexPos(source);
	v3s32 EndIndex    = getIndexPos(destination);
	...
	switch (algo) {
		case PA_DIJKSTRA:
			update_cost_retval = updateAllCosts(StartIndex, v3s32(0, 0, 0), 0, 0);
			break;
		case PA_PLAIN_NP:
		case PA_PLAIN:
	...
	if (update_cost_retval) {
		...
		std::vector<v3s32>::reverse_iterator rit = index_path.rbegin();
...
/******************************************************************************/
v3s32 Pathfinder::getRealPos(v3s32 ipos)
{
	return m_limits.MinEdge + ipos;
}

/******************************************************************************/
PathCost Pathfinder::calcCost(v3s32 pos, v3s32 dir)
{
	PathCost retval;
	...
	if (!m_ndef->get(node_at_pos2).walkable) {
		MapNode node_below_pos2 =
			m_map->getNode(pos2 + v3s32(0, -1, 0));

		//did we get information about node?
		if (node_below_pos2.param0 == CONTENT_IGNORE ) {
				VERBOSE_TARGET << "Pathfinder: (2) area at pos: "
					<< PP((pos2 + v3s32(0, -1, 0))) << " not loaded";
				return retval;
		}
		...
		if (m_ndef->get(node_below_pos2).walkable) {
			...
			v3s32 testpos = pos2 + v3s32(0, -1, 0);
		...
		v3s32 targetpos = pos2; // position for jump target
		v3s32 jumppos = pos; // position for checking if jumping space is free
...
/******************************************************************************/
v3s32 Pathfinder::getIndexPos(v3s32 pos)
{
	return pos - m_limits.MinEdge;
}

/******************************************************************************/
PathGridnode &Pathfinder::getIndexElement(v3s32 ipos)
{
	return m_nodes_container->access(ipos);
}

/******************************************************************************/
inline PathGridnode &Pathfinder::getIdxElem(s32 x, s32 y, s32 z)
{
	return m_nodes_container->access(v3s32(x,y,z));
}

/******************************************************************************/
bool Pathfinder::isValidIndex(v3s32 index)
{
...
/******************************************************************************/
v3s32 Pathfinder::invert(v3s32 pos)
{
	v3s32 retval = pos;
...
/******************************************************************************/
bool Pathfinder::updateAllCosts(v3s32 ipos,
								v3s32 srcdir,
								int current_cost,
								int level)
{
	...
	const static v3s32 directions[4] = {
		v3s32(1,0, 0),
		v3s32(-1,0, 0),
		v3s32(0,0, 1),
		v3s32(0,0,-1)
	...
	for (v3s32 direction : directions) {
		if (direction != srcdir) {
...
/******************************************************************************/
int Pathfinder::getXZManhattanDist(v3s32 pos)
{
...
/******************************************************************************/
bool Pathfinder::updateCostHeuristic(v3s32 isource, v3s32 idestination)
{
	// A* search algorithm.
	...
	std::priority_queue<v3s32, std::vector<v3s32>, PathfinderCompareHeuristic>
			openList(PathfinderCompareHeuristic(this));

	v3s32 source = getRealPos(isource);
	v3s32 destination = getRealPos(idestination);
	...
	while (!openList.empty()) {
		...
		openList.pop();
		v3s32 ipos = getIndexPos(current_pos);
		...
		for (v3s32 direction_flat : directions) {
			...
			v3s32 direction_3d = v3s32(direction_flat);
			direction_3d.Y = cost.y_change;

			// get position of true neighbor
			v3s32 neighbor = current_pos + direction_3d;
			v3s32 ineighbor = getIndexPos(neighbor);
...
/******************************************************************************/
bool Pathfinder::buildPath(std::vector<v3s32> &path, v3s32 ipos)
{
...
/******************************************************************************/
v3s32 Pathfinder::walkDownwards(v3s32 pos, unsigned int max_down) {
	if (max_down == 0)
		return pos;
	v3s32 testpos = v3s32(pos);
	MapNode node_at_pos = m_map->getNode(testpos);
	unsigned int down = 0;
	while ((node_at_pos.param0 != CONTENT_IGNORE) &&
			(!m_ndef->get(node_at_pos).walkable) &&
...
/******************************************************************************/
void Pathfinder::printPath(const std::vector<v3s32> &path)
{
	unsigned int current = 0;
	for (std::vector<v3s32>::iterator i = path.begin();
			i != path.end(); ++i) {
