...
public:
	 ...
	 * @cost cost to set
	 */
	void      setCost(v3size dir, const PathCost &cost);
	...
	v3size     sourcedir;                   /**< origin of movement for current cost   */
	v3size     pos;                         /**< real position of node                 */
...
public:
	virtual PathGridnode &access(v3size p)=0;
	virtual ~GridNodeContainer() = default;

protected:
	Pathfinder *m_pathf;

	void initNode(v3size ipos, PathGridnode *p_node);
};
...
public:
	...
	ArrayGridNodeContainer(Pathfinder *pathf, v3size dimensions);
	virtual PathGridnode &access(v3size p);
...
public:
	...
	virtual PathGridnode &access(v3size p);
private:
	std::map<v3size, PathGridnode> m_nodes;
};
...
public:
	 ...
	 */
	std::vector<v3size> getPath(v3size source,
			v3size destination,
...
private:
	 ...
	 */
	PathGridnode &getIndexElement(v3size ipos);
	 ...
	 * @return true/false
	 */
	bool           isValidIndex(v3size index);
	 ...
	 */
	int           getXZManhattanDist(v3size pos);
	 ...
	 */
	PathCost     calcCost(v3size pos, v3size dir);
	 ...
	 */
	bool          updateAllCosts(v3size ipos, v3size srcdir, int current_cost, int level);
	 ...
	 * @return true/false path to destination has been found
	 */
	bool          updateCostHeuristic(v3size isource, v3size idestination);
	 ...
	 */
	bool          buildPath(std::vector<v3size> &path, v3size ipos);
	 ...
	 * pos is returned
	 */
	v3size         walkDownwards(v3size pos, unsigned int max_down);
	...
	v3size m_start;                /**< source position                          */
	v3size m_destination;          /**< destination position                     */
	 ...
	 */
	void printPath(std::vector<v3size> path);
...
class PathfinderCompareHeuristic
{
	...
	public:
		...
		bool operator() (v3size pos1, v3size pos2) {
			v3size ipos1 = myPathfinder->getIndexPos(pos1);
			v3size ipos2 = myPathfinder->getIndexPos(pos2);
...
std::vector<v3size> get_path(Map* map, const NodeDefManager *ndef,
		v3size source,
		v3size destination,
		unsigned int searchdistance,
		unsigned int max_jump,
...
/******************************************************************************/
PathCost PathGridnode::getCost(v3size dir)
{
	if (dir.X > 0) {
...
/******************************************************************************/
void PathGridnode::setCost(v3size dir, const PathCost &cost)
{
	if (dir.X > 0) {
...
void GridNodeContainer::initNode(v3size ipos, PathGridnode *p_node)
{
	const NodeDefManager *ndef = m_pathf->m_ndef;
	PathGridnode &elem = *p_node;

	v3size realpos = m_pathf->getRealPos(ipos);

	MapNode current = m_pathf->m_map->getNode(realpos);
	MapNode below   = m_pathf->m_map->getNode(realpos + v3size(0, -1, 0));
	...
	if (m_pathf->m_prefetch) {
		elem.directions[DIR_XP] = m_pathf->calcCost(realpos, v3size( 1, 0, 0));
		elem.directions[DIR_XM] = m_pathf->calcCost(realpos, v3size(-1, 0, 0));
		elem.directions[DIR_ZP] = m_pathf->calcCost(realpos, v3size( 0, 0, 1));
		elem.directions[DIR_ZM] = m_pathf->calcCost(realpos, v3size( 0, 0,-1));
	}
}

ArrayGridNodeContainer::ArrayGridNodeContainer(Pathfinder *pathf, v3size dimensions) :
...
PathGridnode &ArrayGridNodeContainer::access(v3size p)
{
...
PathGridnode &MapGridNodeContainer::access(v3size p)
{
	std::map<v3size, PathGridnode>::iterator it = m_nodes.find(p);
	if (it != m_nodes.end()) {
		return it->second;
	}
...
/******************************************************************************/
std::vector<v3size> Pathfinder::getPath(v3size source,
							v3size destination,
							unsigned int searchdistance,
							unsigned int max_jump,
							unsigned int max_drop,
							PathAlgorithm algo)
{
	...
	v3size diff = m_limits.MaxEdge - m_limits.MinEdge;
	...
	//a walkable node.
	v3size true_source = v3size(source);
	...
	v3size true_destination = v3size(destination);
	destination = walkDownwards(destination, m_maxjump);

	//validate and mark start and end pos

	v3size StartIndex  = getIndexPos(source);
	v3size EndIndex    = getIndexPos(destination);
	...
	switch (algo) {
		case PA_DIJKSTRA:
			update_cost_retval = updateAllCosts(StartIndex, v3size(0, 0, 0), 0, 0);
			break;
		case PA_PLAIN_NP:
		case PA_PLAIN:
	...
	if (update_cost_retval) {
		...
		std::vector<v3size>::reverse_iterator rit = index_path.rbegin();
...
/******************************************************************************/
v3size Pathfinder::getRealPos(v3size ipos)
{
	return m_limits.MinEdge + ipos;
}

/******************************************************************************/
PathCost Pathfinder::calcCost(v3size pos, v3size dir)
{
	PathCost retval;
	...
	if (!m_ndef->get(node_at_pos2).walkable) {
		MapNode node_below_pos2 =
			m_map->getNode(pos2 + v3size(0, -1, 0));

		//did we get information about node?
		if (node_below_pos2.param0 == CONTENT_IGNORE ) {
				VERBOSE_TARGET << "Pathfinder: (2) area at pos: "
					<< PP((pos2 + v3size(0, -1, 0))) << " not loaded";
				return retval;
		}
		...
		v3size targetpos = pos2; // position for jump target
		v3size jumppos = pos; // position for checking if jumping space is free
		...
		while ((node_target.param0 != CONTENT_IGNORE) &&
			...
			targetpos += v3size(0, 1, 0);
			jumppos   += v3size(0, 1, 0);
...
/******************************************************************************/
v3size Pathfinder::getIndexPos(v3size pos)
{
	return pos - m_limits.MinEdge;
}

/******************************************************************************/
PathGridnode &Pathfinder::getIndexElement(v3size ipos)
{
...
{
	return m_nodes_container->access(v3size(x,y,z));
}

/******************************************************************************/
bool Pathfinder::isValidIndex(v3size index)
{
...
/******************************************************************************/
v3size Pathfinder::invert(v3size pos)
{
	v3size retval = pos;
...
/******************************************************************************/
bool Pathfinder::updateAllCosts(v3size ipos,
								v3size srcdir,
								int current_cost,
								int level)
{
	...
	const static v3size directions[4] = {
		v3size(1,0, 0),
		v3size(-1,0, 0),
		v3size(0,0, 1),
		v3size(0,0,-1)
	...
	for (v3size direction : directions) {
		if (direction != srcdir) {
...
/******************************************************************************/
int Pathfinder::getXZManhattanDist(v3size pos)
{
...
/******************************************************************************/
bool Pathfinder::updateCostHeuristic(v3size isource, v3size idestination)
{
	// A* search algorithm.
	...
	std::priority_queue<v3size, std::vector<v3size>, PathfinderCompareHeuristic>
			openList(PathfinderCompareHeuristic(this));

	v3size source = getRealPos(isource);
	v3size destination = getRealPos(idestination);
	...
	while (!openList.empty()) {
		...
		openList.pop();
		v3size ipos = getIndexPos(current_pos);
		...
		for (v3size direction_flat : directions) {
			...
			v3size direction_3d = v3size(direction_flat);
			direction_3d.Y = cost.y_change;

			// get position of true neighbor
			v3size neighbor = current_pos + direction_3d;
			v3size ineighbor = getIndexPos(neighbor);
...
/******************************************************************************/
bool Pathfinder::buildPath(std::vector<v3size> &path, v3size ipos)
{
...
/******************************************************************************/
v3size Pathfinder::walkDownwards(v3size pos, unsigned int max_down) {
	if (max_down == 0)
		return pos;
	v3size testpos = v3size(pos);
	MapNode node_at_pos = m_map->getNode(testpos);
	unsigned int down = 0;
	while ((node_at_pos.param0 != CONTENT_IGNORE) &&
			(!m_ndef->get(node_at_pos).walkable) &&
...
/******************************************************************************/
void Pathfinder::printPath(const std::vector<v3size> &path)
{
	unsigned int current = 0;
	for (std::vector<v3size>::iterator i = path.begin();
			i != path.end(); ++i) {
