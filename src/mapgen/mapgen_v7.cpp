...
MapgenV7Params::MapgenV7Params():
	np_terrain_base      (4.0,   70.0,  v3d(600,  600,  600),  82341, 5, 0.6,  2.0),
	np_terrain_alt       (4.0,   25.0,  v3d(600,  600,  600),  5934,  5, 0.6,  2.0),
	np_terrain_persist   (0.6,   0.1,   v3d(2000, 2000, 2000), 539,   3, 0.6,  2.0),
	np_height_select     (-8.0,  16.0,  v3d(500,  500,  500),  4213,  6, 0.7,  2.0),
	np_filler_depth      (0.0,   1.2,   v3d(150,  150,  150),  261,   3, 0.7,  2.0),
	np_mount_height      (256.0, 112.0, v3d(1000, 1000, 1000), 72449, 3, 0.6,  2.0),
	np_ridge_uwater      (0.0,   1.0,   v3d(1000, 1000, 1000), 85039, 5, 0.6,  2.0),
	np_mountain          (-0.6,  1.0,   v3d(250,  350,  250),  5333,  5, 0.63, 2.0),
	np_ridge             (0.0,   1.0,   v3d(100,  100,  100),  6467,  4, 0.75, 2.0),
	np_floatland         (0.0,   0.7,   v3d(384,  96,   384),  1009,  4, 0.75, 1.618),
	np_cavern            (0.0,   1.0,   v3d(384,  128,  384),  723,   5, 0.63, 2.0),
	np_cave1             (0.0,   12.0,  v3d(61,   61,   61),   52534, 3, 0.5,  2.0),
	np_cave2             (0.0,   12.0,  v3d(67,   67,   67),   10325, 3, 0.5,  2.0),
	np_dungeons          (0.9,   0.5,   v3d(500,  500,  500),  0,     2, 0.8,  2.0)
{
}
...
void MapgenV7Params::readParams(const Settings *settings)
{
	...
	settings->getS32NoEx("mgv7_mount_zero_level",       mount_zero_level);
	settings->getS32NoEx("mgv7_floatland_ymin",         floatland_ymin);
	settings->getS32NoEx("mgv7_floatland_ymax",         floatland_ymax);
	settings->getS32NoEx("mgv7_floatland_taper",        floatland_taper);
	settings->getFloatNoEx("mgv7_float_taper_exp",      float_taper_exp);
	settings->getFloatNoEx("mgv7_floatland_density",    floatland_density);
	settings->getS32NoEx("mgv7_floatland_ywater",       floatland_ywater);

	settings->getFloatNoEx("mgv7_cave_width",           cave_width);
	settings->getS32NoEx("mgv7_large_cave_depth",       large_cave_depth);
	...
	settings->getS32NoEx("mgv7_cavern_limit",           cavern_limit);
	settings->getS32NoEx("mgv7_cavern_taper",           cavern_taper);
	settings->getFloatNoEx("mgv7_cavern_threshold",     cavern_threshold);
	settings->getS32NoEx("mgv7_dungeon_ymin",           dungeon_ymin);
	settings->getS32NoEx("mgv7_dungeon_ymax",           dungeon_ymax);
...
int MapgenV7::getSpawnLevelAtPoint(v2s32 p)
{
	...
	s32 max_spawn_y = std::fmax(std::fmax(noise_terrain_alt->np.offset,
			noise_terrain_base->np.offset),
			water_level + 16);
	// Base terrain calculation
	s32 y = baseTerrainLevelAtPoint(p.X, p.Y);
...
void MapgenV7::makeChunk(BlockMakeData *data)
{
	...
	s32 stone_surface_max_y = generateTerrain();
	...
	if (flags & MG_LIGHT)
		calcLighting(node_min - v3s32(0, 1, 0), node_max + v3s32(0, 1, 0),
...
float MapgenV7::baseTerrainLevelAtPoint(s32 x, s32 z)
{
...
bool MapgenV7::getMountainTerrainAtPoint(s32 x, s32 y, s32 z)
{
	float mnt_h_n =
...
bool MapgenV7::getMountainTerrainFromMap(int idx_xyz, int idx_xz, s32 y)
{
...
bool MapgenV7::getRiverChannelFromMap(int idx_xyz, int idx_xz, s32 y)
{
...
int MapgenV7::generateTerrain()
{
	...
	s32 float_taper_ymax = floatland_ymax - floatland_taper;
	s32 float_taper_ymin = floatland_ymin + floatland_taper;
	...
	if ((spflags & MGV7_FLOATLANDS) &&
		...
		for (s32 y = node_min.Y - 1; y <= node_max.Y + 1; y++, cache_index++) {
	...
	s32 stone_surface_max_y = -MAX_MAP_GENERATION_LIMIT;
	u32 index2d = 0;

	for (s32 z = node_min.Z; z <= node_max.Z; z++)
	for (s32 x = node_min.X; x <= node_max.X; x++, index2d++) {
		s32 surface_y = baseTerrainLevelFromMap(index2d);
		...
		for (s32 y = node_min.Y - 1; y <= node_max.Y + 1;
				y++,
				index3d += ystride,
