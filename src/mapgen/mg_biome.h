...
public:
	...
	inline void assertChunkSize(v3s32 expect) const
	{
	...
	virtual Biome *calcBiomeAtPoint(v3s32 pos) const = 0;
	...
	virtual void calcBiomeNoise(v3s32 pmin) = 0;
...
struct BiomeParamsOriginal : public BiomeParams {
	BiomeParamsOriginal() :
		np_heat(50, 50, v3d(1000.0, 1000.0, 1000.0), 5349, 3, 0.5, 2.0),
		np_humidity(50, 50, v3d(1000.0, 1000.0, 1000.0), 842, 3, 0.5, 2.0),
		np_heat_blend(0, 1.5, v3d(8.0, 8.0, 8.0), 13, 2, 1.0, 2.0),
		np_humidity_blend(0, 1.5, v3d(8.0, 8.0, 8.0), 90003, 2, 1.0, 2.0)
	{
	}
...
public:
	BiomeGenOriginal(BiomeManager *biomemgr,
		const BiomeParamsOriginal *params, v3s32 chunksize);
	...
	float calcHeatAtPoint(v3s32 pos) const;
	float calcHumidityAtPoint(v3s32 pos) const;
	Biome *calcBiomeAtPoint(v3s32 pos) const;

	void calcBiomeNoise(v3s32 pmin);

	biome_t *getBiomes(s32 *heightmap, v3s32 pmin);
	Biome *getBiomeAtPoint(v3s32 pos) const;
	Biome *getBiomeAtIndex(size_t index, v3s32 pos) const;

	Biome *calcBiomeFromNoise(float heat, float humidity, v3s32 pos) const;
...
public:
	...
	BiomeGen *createBiomeGen(BiomeGenType type, BiomeParams *params, v3s32 chunksize)
	{
		switch (type) {
