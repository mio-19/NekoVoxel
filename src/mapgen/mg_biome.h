...
public:
	...
	inline void assertChunkSize(v3size expect) const
	{
	...
	virtual Biome *calcBiomeAtPoint(v3size pos) const = 0;
	...
	virtual void calcBiomeNoise(v3size pmin) = 0;
...
public:
	BiomeGenOriginal(BiomeManager *biomemgr,
		const BiomeParamsOriginal *params, v3size chunksize);
	...
	float calcHeatAtPoint(v3size pos) const;
	float calcHumidityAtPoint(v3size pos) const;
	Biome *calcBiomeAtPoint(v3size pos) const;

	void calcBiomeNoise(v3size pmin);

	biome_t *getBiomes(s16 *heightmap, v3size pmin);
	Biome *getBiomeAtPoint(v3size pos) const;
	Biome *getBiomeAtIndex(size_t index, v3size pos) const;

	Biome *calcBiomeFromNoise(float heat, float humidity, v3size pos) const;
...
public:
	...
	BiomeGen *createBiomeGen(BiomeGenType type, BiomeParams *params, v3size chunksize)
	{
		switch (type) {
