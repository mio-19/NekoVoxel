...
void Schematic::blitToVManip(MMVManip *vm, v3size p, Rotation rot, bool force_place)
{
...
bool Schematic::placeOnVManip(MMVManip *vm, v3size p, u32 flags,
	Rotation rot, bool force_place)
{
	...
	v3size s = (rot == ROTATE_90 || rot == ROTATE_270) ?
		v3size(size.Z, size.Y, size.X) : size;
	...
	return vm->m_area.contains(VoxelArea(p, p + s - v3size(1, 1, 1)));
}

void Schematic::placeOnMap(ServerMap *map, v3size p, u32 flags,
	Rotation rot, bool force_place)
{
	std::map<v3size, MapBlock *> modified_blocks;
	std::map<v3size, MapBlock *>::iterator it;
	...
	v3size bp1 = getNodeBlockPos(p);
	v3size bp2 = getNodeBlockPos(p + s - v3size(1, 1, 1));
...
bool Schematic::getSchematicFromMap(Map *map, v3size p1, v3size p2)
{
