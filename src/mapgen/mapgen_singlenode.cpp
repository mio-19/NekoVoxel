...
void MapgenSinglenode::makeChunk(BlockMakeData *data)
{
	...
	v3s32 node_min = blockpos_min * MAP_BLOCKSIZE;
	v3s32 node_max = (blockpos_max + v3s32(1, 1, 1)) * MAP_BLOCKSIZE - v3s32(1, 1, 1);
	...
	for (s32 y = node_min.Y; y <= node_max.Y; y++) {
		...
		for (s32 x = node_min.X; x <= node_max.X; x++) {
...
int MapgenSinglenode::getSpawnLevelAtPoint(v2s32 p)
{
	return 0;
}
