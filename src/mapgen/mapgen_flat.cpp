...
MapgenFlatParams::MapgenFlatParams():
	np_terrain      (0,   1,   v3d(600, 600, 600), 7244,  5, 0.6,  2.0),
	np_filler_depth (0,   1.2, v3d(150, 150, 150), 261,   3, 0.7,  2.0),
...
void MapgenFlatParams::readParams(const Settings *settings)
{
	...
	settings->getS32NoEx("mgflat_ground_level",         ground_level);
	settings->getS32NoEx("mgflat_large_cave_depth",     large_cave_depth);
	...
	settings->getS32NoEx("mgflat_cavern_limit",         cavern_limit);
	settings->getS32NoEx("mgflat_cavern_taper",         cavern_taper);
	settings->getFloatNoEx("mgflat_cavern_threshold",   cavern_threshold);
	settings->getS32NoEx("mgflat_dungeon_ymin",         dungeon_ymin);
	settings->getS32NoEx("mgflat_dungeon_ymax",         dungeon_ymax);
...
void MapgenFlatParams::writeParams(Settings *settings) const
{
	...
	settings->setS32("mgflat_ground_level",         ground_level);
	settings->setS32("mgflat_large_cave_depth",     large_cave_depth);
	...
	settings->setS32("mgflat_cavern_limit",         cavern_limit);
	settings->setS32("mgflat_cavern_taper",         cavern_taper);
	settings->setFloat("mgflat_cavern_threshold",   cavern_threshold);
	settings->setS32("mgflat_dungeon_ymin",         dungeon_ymin);
	settings->setS32("mgflat_dungeon_ymax",         dungeon_ymax);
...
int MapgenFlat::getSpawnLevelAtPoint(v2s32 p)
{
	s32 stone_level = ground_level;
	float n_terrain =
	...
	if ((spflags & MGFLAT_LAKES) && n_terrain < lake_threshold) {
		s32 depress = (lake_threshold - n_terrain) * lake_steepness;
		stone_level = ground_level - depress;
	} else if ((spflags & MGFLAT_HILLS) && n_terrain > hill_threshold) {
		s32 rise = (n_terrain - hill_threshold) * hill_steepness;
...
void MapgenFlat::makeChunk(BlockMakeData *data)
{
	...
	//setLighting(node_min - v3s32(1, 0, 1) * MAP_BLOCKSIZE,
	//			node_max + v3s32(1, 0, 1) * MAP_BLOCKSIZE, 0xFF);

	this->generating = false;
}


s32 MapgenFlat::generateTerrain()
{
	...
	for (s32 x = node_min.X; x <= node_max.X; x++, ni2d++) {
		s32 stone_level = ground_level;
