...
MapgenV5Params::MapgenV5Params():
	np_filler_depth (0,   1,   v3d(150, 150, 150), 261,    4, 0.7,  2.0),
	np_factor       (0,   1,   v3d(250, 250, 250), 920381, 3, 0.45, 2.0),
	np_height       (0,   10,  v3d(250, 250, 250), 84174,  4, 0.5,  2.0),
	np_ground       (0,   40,  v3d(80,  80,  80),  983240, 4, 0.55, 2.0, NOISE_FLAG_EASED),
...
void MapgenV5Params::readParams(const Settings *settings)
{
	...
	settings->getS32NoEx("mgv5_large_cave_depth",     large_cave_depth);
	...
	settings->getS32NoEx("mgv5_cavern_limit",         cavern_limit);
	settings->getS32NoEx("mgv5_cavern_taper",         cavern_taper);
	settings->getFloatNoEx("mgv5_cavern_threshold",   cavern_threshold);
	settings->getS32NoEx("mgv5_dungeon_ymin",         dungeon_ymin);
	settings->getS32NoEx("mgv5_dungeon_ymax",         dungeon_ymax);
...
int MapgenV5::getSpawnLevelAtPoint(v2s32 p)
{
	...
	s32 max_spawn_y = MYMAX(noise_height->np.offset, water_level + 16);
...
void MapgenV5::makeChunk(BlockMakeData *data)
{
	...
	s32 stone_surface_max_y = generateBaseTerrain();
...
int MapgenV5::generateBaseTerrain()
{
	...
	for (s32 z=node_min.Z; z<=node_max.Z; z++) {
		for (s32 y=node_min.Y - 1; y<=node_max.Y + 1; y++) {
			...
			for (s32 x=node_min.X; x<=node_max.X; x++, vi++, index++, index2d++) {
