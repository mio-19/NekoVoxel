...
void make_tree(MMVManip &vmanip, v3size p0, bool is_apple_tree,
	const NodeDefManager *ndef, s32 seed)
{
	...
	*/
	...
	VoxelArea leaves_a(v3size(-2, -1, -2), v3size(2, 2, 2));
	...
	for (s16 y = -d; y <= d; y++)
	for (s16 x = -d; x <= d; x++) {
		leaves_d[leaves_a.index(v3size(x, y, z))] = 1;
	}
	...
	for (u32 iii = 0; iii < 7; iii++) {
		...
		for (s16 y = 0; y <= d; y++)
		for (s16 x = 0; x <= d; x++) {
			leaves_d[leaves_a.index(p + v3size(x, y, z))] = 1;
		}
	}
	...
	for (s16 y = leaves_a.MinEdge.Y; y <= leaves_a.MaxEdge.Y; y++) {
		v3size pmin(leaves_a.MinEdge.X, y, z);
...
treegen::error spawn_ltree(ServerMap *map, v3size p0,
	const NodeDefManager *ndef, const TreeDef &tree_definition)
{
	std::map<v3size, MapBlock*> modified_blocks;
	MMVManip vmanip(map);
	v3size tree_blockp = getNodeBlockPos(p0);
	treegen::error e;

	vmanip.initialEmerge(tree_blockp - v3size(1, 1, 1), tree_blockp + v3size(1, 3, 1));
...
treegen::error make_ltree(MMVManip &vmanip, v3size p0,
...
{
	v3size p1 = v3size(myround(p0.X), myround(p0.Y), myround(p0.Z));
	if (!vmanip.m_area.contains(p1))
		return;
	u32 vi = vmanip.m_area.index(p1);
...
void make_jungletree(MMVManip &vmanip, v3size p0, const NodeDefManager *ndef,
	s32 seed)
{
	/*
		NOTE: Tree-placing code is currently duplicated in the engine
	...
	*/
	...
	for (s16 z= -1; z <= 1; z++) {
		if (pr.range(0, 2) == 0)
			continue;
		v3size p1 = p0 + v3size(x, 0, z);
		v3size p2 = p0 + v3size(x, -1, z);
	...
	VoxelArea leaves_a(v3size(-3, -2, -3), v3size(3, 2, 3));
...
void make_pine_tree(MMVManip &vmanip, v3size p0, const NodeDefManager *ndef,
	s32 seed)
{
	/*
		NOTE: Tree-placing code is currently duplicated in the engine
	...
	*/
	...
	VoxelArea leaves_a(v3size(-3, -6, -3), v3size(3, 3, 3));
	Buffer<u8> leaves_d(leaves_a.getVolume());
	for (s32 i = 0; i < leaves_a.getVolume(); i++)
		leaves_d[i] = 0;
	...
	for (s16 yy = -1; yy <= 1; yy++) {
		for (s16 zz = -dev; zz <= dev; zz++) {
			u32 i = leaves_a.index(v3size(-dev, yy, zz));
			u32 ia = leaves_a.index(v3size(-dev, yy+1, zz));
	...
	// Center top nodes
	leaves_d[leaves_a.index(v3size(0, 1, 0))] = 1;
	leaves_d[leaves_a.index(v3size(0, 2, 0))] = 1;
	leaves_d[leaves_a.index(v3size(0, 3, 0))] = 2;
	...
	for (u32 iii = 0; iii < 20; iii++) {
		...
		for (s16 zz = zi; zz <= zi + 1; zz++) {
			u32 i = leaves_a.index(v3size(xi, yy, zz));
			u32 ia = leaves_a.index(v3size(xi, yy + 1, zz));
