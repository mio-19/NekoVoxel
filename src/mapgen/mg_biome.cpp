	...
	ObjDefManager(server, OBJDEF_BIOME)
{
	...
	b->depth_water_top = 0;
	b->depth_riverbed  = 0;
	b->min_pos         = v3size(-MAX_MAP_GENERATION_LIMIT,
			-MAX_MAP_GENERATION_LIMIT, -MAX_MAP_GENERATION_LIMIT);
	b->max_pos         = v3size(MAX_MAP_GENERATION_LIMIT,
			MAX_MAP_GENERATION_LIMIT, MAX_MAP_GENERATION_LIMIT);
	b->heat_point      = 0.0;
...
float BiomeGenOriginal::calcHeatAtPoint(v3size pos) const
{
...
float BiomeGenOriginal::calcHumidityAtPoint(v3size pos) const
{
	return NoisePerlin2D(&m_params->np_humidity, pos.X, pos.Z, m_params->seed) +
		NoisePerlin2D(&m_params->np_humidity_blend, pos.X, pos.Z, m_params->seed);
}

Biome *BiomeGenOriginal::calcBiomeAtPoint(v3size pos) const
{
	return calcBiomeFromNoise(calcHeatAtPoint(pos), calcHumidityAtPoint(pos), pos);
}


void BiomeGenOriginal::calcBiomeNoise(v3size pmin)
{
	m_pmin = pmin;
...
biome_t *BiomeGenOriginal::getBiomes(s16 *heightmap, v3size pmin)
{
	...
	for (s16 xr = 0; xr < m_csize.X; xr++) {
		...
		Biome *biome = calcBiomeFromNoise(
			...
			v3size(pmin.X + xr, heightmap[i], pmin.Z + zr));
...
Biome *BiomeGenOriginal::getBiomeAtPoint(v3size pos) const
{
...
Biome *BiomeGenOriginal::getBiomeAtIndex(size_t index, v3size pos) const
{
...
Biome *BiomeGenOriginal::calcBiomeFromNoise(float heat, float humidity, v3size pos) const
{
