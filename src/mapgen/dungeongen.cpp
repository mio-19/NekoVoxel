	...
	GenerateNotifier *gennotify, DungeonParams *dparams)
{
	...
	if (dparams) {
		...
		dp.holesize            = v3size(1, 2, 1);
		dp.corridor_len_min    = 1;
		dp.corridor_len_max    = 13;
		dp.room_size_min       = v3size(4, 4, 4);
		dp.room_size_max       = v3size(8, 6, 8);
		dp.room_size_large_min = v3size(8, 8, 8);
...
void DungeonGen::generate(MMVManip *vm, u32 bseed, v3size nmin, v3size nmax)
{
	...
	for (u32 i = 0; i < dp.num_dungeons; i++)
		makeDungeon(v3size(1, 1, 1) * MAP_BLOCKSIZE);
...
void DungeonGen::makeDungeon(v3size start_padding)
{
	const v3size &areasize = vm->m_area.getExtent();
	v3size roomsize;
	v3size roomplace;
	...
	for (u32 i = 0; i < 100 && !fits; i++) {
		...
		*/
		...
		for (s16 x = 0; x < roomsize.X; x++) {
			v3size p = roomplace + v3size(x, y, z);
	...
	*/
	v3size last_room_center = roomplace + v3size(roomsize.X / 2, 1, roomsize.Z / 2);

	for (u32 i = 0; i < dp.num_rooms; i++) {
		// Make a room to the determined place
		makeRoom(roomsize, roomplace);

		v3size room_center = roomplace + v3size(roomsize.X / 2, 1, roomsize.Z / 2);
		if (gennotify)
...
void DungeonGen::makeRoom(v3size roomsize, v3size roomplace)
{
	...
	for (s16 y = 0; y < roomsize.Y; y++) {
		{
			v3size p = roomplace + v3size(0, y, z);
		...
		{
			v3size p = roomplace + v3size(roomsize.X - 1, y, z);
			if (!vm->m_area.contains(p))
				continue;
			u32 vi = vm->m_area.index(p);
			if (vm->m_flags[vi] & VMANIP_FLAG_DUNGEON_UNTOUCHABLE)
	...
	for (s16 y = 0; y < roomsize.Y; y++) {
		{
			v3size p = roomplace + v3size(x, y, 0);
			if (!vm->m_area.contains(p))
				continue;
			u32 vi = vm->m_area.index(p);
			if (vm->m_flags[vi] & VMANIP_FLAG_DUNGEON_UNTOUCHABLE)
		...
		{
			v3size p = roomplace + v3size(x, y, roomsize.Z - 1);
			if (!vm->m_area.contains(p))
				continue;
			u32 vi = vm->m_area.index(p);
			if (vm->m_flags[vi] & VMANIP_FLAG_DUNGEON_UNTOUCHABLE)
	...
	for (s16 x = 0; x < roomsize.X; x++) {
		{
			v3size p = roomplace + v3size(x, 0, z);
			if (!vm->m_area.contains(p))
				continue;
			u32 vi = vm->m_area.index(p);
			if (vm->m_flags[vi] & VMANIP_FLAG_DUNGEON_UNTOUCHABLE)
		...
		{
			v3size p = roomplace + v3size(x,roomsize. Y - 1, z);
			if (!vm->m_area.contains(p))
				continue;
			u32 vi = vm->m_area.index(p);
			if (vm->m_flags[vi] & VMANIP_FLAG_DUNGEON_UNTOUCHABLE)
...
void DungeonGen::makeFill(v3size place, v3size size,
	u8 avoid_flags, MapNode n, u8 or_flags)
{
	for (s16 z = 0; z < size.Z; z++)
	for (s16 y = 0; y < size.Y; y++)
	for (s16 x = 0; x < size.X; x++) {
		v3size p = place + v3size(x, y, z);
		if (!vm->m_area.contains(p))
			continue;
		u32 vi = vm->m_area.index(p);
...
void DungeonGen::makeHole(v3size place)
{
...
void DungeonGen::makeDoor(v3size doorplace, v3size doordir)
{
	makeHole(doorplace);
...
void DungeonGen::makeCorridor(v3size doorplace, v3size doordir,
	v3size &result_place, v3size &result_dir)
{
	...
	for (u32 i = 0; i < length; i++) {
		...
		if (vm->m_area.contains(p) && vm->m_area.contains(p + v3size(0, 1, 0))) {
			if (make_stairs) {
				makeFill(p + v3size(-1, -1, -1),
					dp.holesize + v3size(2, 3, 2),
				...
				if (((dir.X ^ dir.Z) & 1) &&
					...
					v3size swv = (dir.Z != 0) ? v3size(1, 0, 0) : v3size(0, 0, 1);

					for (u16 st = 0; st < stair_width; st++) {
						if (make_stairs == -1) {
							u32 vi = vm->m_area.index(ps.X - dir.X, ps.Y - 1, ps.Z - dir.Z);
							if (vm->m_area.contains(ps + v3size(-dir.X, -1, -dir.Z)) &&
							...
							if (vm->m_area.contains(ps + v3size(0, -1, 0)) &&
									vm->m_data[vi].getContent() == dp.c_wall) {
								vm->m_flags[vi] |= VMANIP_FLAG_DUNGEON_UNTOUCHABLE;
								vm->m_data[vi] = MapNode(dp.c_stair, 0, facedir);
							}
...
bool DungeonGen::findPlaceForDoor(v3size &result_place, v3size &result_dir)
{
	for (u32 i = 0; i < 100; i++) {
		v3size p = m_pos + m_dir;
		v3size p1 = p + v3size(0, 1, 0);
		...
		*/
		...
		if (vm->getNodeNoExNoEmerge(p +
				v3size(0, 0, 0)).getContent() == dp.c_wall &&
				vm->getNodeNoExNoEmerge(p +
				v3size(0, 1, 0)).getContent() == CONTENT_AIR &&
				vm->getNodeNoExNoEmerge(p +
				v3size(0, 2, 0)).getContent() == CONTENT_AIR)
			p += v3size(0,1,0);
		// Jump one down if the actual space is there
		if (vm->getNodeNoExNoEmerge(p +
				v3size(0, 1, 0)).getContent() == dp.c_wall &&
				vm->getNodeNoExNoEmerge(p +
				v3size(0, 0, 0)).getContent() == CONTENT_AIR &&
				vm->getNodeNoExNoEmerge(p +
				v3size(0, -1, 0)).getContent() == CONTENT_AIR)
			p += v3size(0, -1, 0);
...
bool DungeonGen::findPlaceForRoomDoor(v3size roomsize, v3size &result_doorplace,
	v3size &result_doordir, v3size &result_roomplace)
{
	for (s16 trycount = 0; trycount < 30; trycount++) {
		...
		v3size roomplace;
		// X east, Z north, Y up
		if (doordir == v3size(1, 0, 0)) // X+
			roomplace = doorplace +
				v3size(0, -1, random.range(-roomsize.Z + 2, -2));
		if (doordir == v3size(-1, 0, 0)) // X-
			roomplace = doorplace +
				v3size(-roomsize.X + 1, -1, random.range(-roomsize.Z + 2, -2));
		if (doordir == v3size(0, 0, 1)) // Z+
			roomplace = doorplace +
				v3size(random.range(-roomsize.X + 2, -2), -1, 0);
		if (doordir == v3size(0, 0, -1)) // Z-
			roomplace = doorplace +
				v3size(random.range(-roomsize.X + 2, -2), -1, -roomsize.Z + 1);
...
v3size rand_ortho_dir(PseudoRandom &random, bool diagonal_dirs)
{
	...
	if (random.next() % 2 == 0)
		return random.next() % 2 ? v3size(-1, 0, 0) : v3size(1, 0, 0);

	return random.next() % 2 ? v3size(0, 0, -1) : v3size(0, 0, 1);
}
