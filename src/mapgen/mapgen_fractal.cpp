...
MapgenFractalParams::MapgenFractalParams():
	np_seabed       (-14, 9,   v3d(600, 600, 600), 41900, 5, 0.6, 2.0),
...
void MapgenFractalParams::readParams(const Settings *settings)
{
	...
	settings->getS32NoEx("mgfractal_large_cave_depth",     large_cave_depth);
	...
	settings->getS32NoEx("mgfractal_dungeon_ymin",         dungeon_ymin);
	settings->getS32NoEx("mgfractal_dungeon_ymax",         dungeon_ymax);
...
int MapgenFractal::getSpawnLevelAtPoint(v2s32 p)
{
	...
	s32 search_start = 0; // No terrain search start

	// If terrain present, don't start search below terrain or water level
	if (noise_seabed) {
		s32 seabed_level = NoisePerlin2D(&noise_seabed->np, p.X, p.Y, seed);
		search_start = MYMAX(search_start, MYMAX(seabed_level, water_level));
	}

	for (s32 y = search_start; y <= search_start + 4096; y++) {
...
bool MapgenFractal::getFractalAtPoint(s32 x, s32 y, s32 z)
{
...
s32 MapgenFractal::generateTerrain()
{
	...
	for (s32 z = node_min.Z; z <= node_max.Z; z++) {
		for (s32 y = node_min.Y - 1; y <= node_max.Y + 1; y++) {
			...
			for (s32 x = node_min.X; x <= node_max.X; x++, vi++, index2d++) {
				if (vm->m_data[vi].getContent() != CONTENT_IGNORE)
					continue;

				s32 seabed_height = -MAX_MAP_GENERATION_LIMIT;
				if (noise_seabed)
