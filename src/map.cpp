...
MapSector * Map::getSectorNoGenerateNoLock(v2size p)
{
...
MapSector * Map::getSectorNoGenerate(v2size p)
{
	return getSectorNoGenerateNoLock(p);
}

MapBlock * Map::getBlockNoCreateNoEx(v3size p3d)
{
	v2size p2d(p3d.X, p3d.Z);
...
MapBlock * Map::getBlockNoCreate(v3size p3d)
{
...
bool Map::isValidPosition(v3size p)
{
	v3size blockpos = getNodeBlockPos(p);
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
	return (block != NULL);
}

// Returns a CONTENT_IGNORE node if not found
MapNode Map::getNode(v3size p, bool *is_valid_position)
{
	v3size blockpos = getNodeBlockPos(p);
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
	if (block == NULL) {
	...
	v3size relpos = p - blockpos*MAP_BLOCKSIZE;
...
static void set_node_in_block(MapBlock *block, v3size relpos, MapNode n)
{
	...
	if(n.getContent() == CONTENT_IGNORE){
		...
		v3size blockpos = block->getPos();
		v3size p = blockpos * MAP_BLOCKSIZE + relpos;
...
void Map::setNode(v3size p, MapNode n)
{
	v3size blockpos = getNodeBlockPos(p);
...
void Map::addNodeAndUpdate(v3size p, MapNode n,
		...
		bool remove_metadata)
{
	...
	if (f == oldf) {
		...
		// Update lighting
		std::vector<std::pair<v3size, MapNode> > oldnodes;
...
void Map::removeNodeAndUpdate(v3size p,
		std::map<v3size, MapBlock*> &modified_blocks)
{
	addNodeAndUpdate(p, MapNode(CONTENT_AIR), modified_blocks, true);
}

bool Map::addNodeWithEvent(v3size p, MapNode n, bool remove_metadata)
{
	MapEditEvent event;
...
bool Map::removeNodeWithEvent(v3size p)
{
	MapEditEvent event;
...
void Map::timerUpdate(float dtime, float unload_timeout, s32 max_loaded_blocks,
		std::vector<v3size> *unloaded_blocks)
{
	...
	std::vector<v2size> sector_deletion_queue;
...
void Map::unloadUnreferencedBlocks(std::vector<v3size> *unloaded_blocks)
{
	timerUpdate(0.0, -1.0, 0, unloaded_blocks);
}

void Map::deleteSectors(std::vector<v2size> &sectorList)
{
	for (v2size j : sectorList) {
...
const static v3size liquid_6dirs[6] = {
...
struct NodeNeighbor {
	...
	NodeNeighbor(const MapNode &node, NeighborType n_type, const v3size &pos)
		: n(node),
		  t(n_type),
		  p(pos)
	{ }
...
void ServerMap::transforming_liquid_add(v3size p) {
        m_transforming_liquid.push_back(p);
}

void ServerMap::transformLiquids(std::map<v3size, MapBlock*> &modified_blocks,
		ServerEnvironment *env)
{
	...
	std::vector<v3size> must_reflow;

	std::vector<std::pair<v3size, MapNode> > changed_nodes;

	std::vector<v3size> check_for_falling;
	...
	while (m_transforming_liquid.size() != 0)
	{
		...
		*/
		v3size p0 = m_transforming_liquid.front();
		...
		v3size blockpos = getNodeBlockPos(p0);
		MapBlock *block = getBlockNoCreateNoEx(blockpos);
		if (block != NULL) {
			modified_blocks[blockpos] =  block;
	...
	for (const v3size &p : check_for_falling) {
...
std::vector<v3size> Map::findNodesWithMetadata(v3size p1, v3size p2)
{
	std::vector<v3size> positions_with_meta;

	sortBoxVerticies(p1, p2);
	v3size bpmin = getNodeBlockPos(p1);
	v3size bpmax = getNodeBlockPos(p2);
	...
	for (s_size x = bpmin.X; x <= bpmax.X; x++) {
		...
		v3size p_base = blockpos * MAP_BLOCKSIZE;
		std::vector<v3size> keys = block->m_node_metadata.getAllKeys();
		for (size_t i = 0; i != keys.size(); i++) {
			v3size p(keys[i] + p_base);
			if (!area.contains(p))
				continue;
...
NodeMetadata *Map::getNodeMetadata(v3size p)
{
	v3size blockpos = getNodeBlockPos(p);
	v3size p_rel = p - blockpos*MAP_BLOCKSIZE;
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
	if(!block){
...
bool Map::setNodeMetadata(v3size p, NodeMetadata *meta)
{
	v3size blockpos = getNodeBlockPos(p);
	v3size p_rel = p - blockpos*MAP_BLOCKSIZE;
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
...
void Map::removeNodeMetadata(v3size p)
{
	v3size blockpos = getNodeBlockPos(p);
	v3size p_rel = p - blockpos*MAP_BLOCKSIZE;
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
...
NodeTimer Map::getNodeTimer(v3size p)
{
	v3size blockpos = getNodeBlockPos(p);
	v3size p_rel = p - blockpos*MAP_BLOCKSIZE;
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
...
void Map::removeNodeTimer(v3size p)
{
	v3size blockpos = getNodeBlockPos(p);
	v3size p_rel = p - blockpos*MAP_BLOCKSIZE;
	MapBlock *block = getBlockNoCreateNoEx(blockpos);
...
#define CLOSEST_EDGE(pos, bounds, axis) \
	...
	// diagonal
	if (x_inside) {
		check = v3size(pos_camera.X, 0, 0);
		check.Y = CLOSEST_EDGE(pos_camera, block_bounds, Y);
		check.Z = CLOSEST_EDGE(pos_camera, block_bounds, Z);
		return true;
	} else if (y_inside) {
		check = v3size(0, pos_camera.Y, 0);
		check.X = CLOSEST_EDGE(pos_camera, block_bounds, X);
		check.Z = CLOSEST_EDGE(pos_camera, block_bounds, Z);
		return true;
	} else if (z_inside) {
		check = v3size(0, 0, pos_camera.Z);
		check.X = CLOSEST_EDGE(pos_camera, block_bounds, X);
		check.Y = CLOSEST_EDGE(pos_camera, block_bounds, Y);
		return true;
	}
...
bool Map::isOccluded(const v3size &pos_camera, const v3size &pos_target,
	float step, float stepfac, float offset, float end_offset, u32 needed_count)
{
	...
	for (; offset < distance + end_offset; offset += step) {
		...
		v3size pos_node = floatToInt(pos_node_f, BS);
...
bool Map::isBlockOccluded(MapBlock *block, v3size cam_pos_nodes)
{
	...
	v3size pos_blockcenter = block->getPosRelative() + (MAP_BLOCKSIZE / 2);
...
bool ServerMap::initBlockMake(v3size blockpos, BlockMakeData *data)
{
	...
	v3size extra_borders(1, 1, 1);
	v3size full_bpmin = bpmin - extra_borders;
	v3size full_bpmax = bpmax + extra_borders;
...
{
	v3size bpmin = data->blockpos_min;
	v3size bpmax = data->blockpos_max;
...
MapSector *ServerMap::createSector(v2size p2d)
{
	/*
	...
	*/
	if (blockpos_over_max_limit(v3size(p2d.X, 0, p2d.Y)))
...
MapBlock * ServerMap::createBlock(v3size p)
{
	/*
		Do not create over max mapgen limit
	*/
...
MapBlock * ServerMap::emergeBlock(v3size p, bool create_blank)
{
	{
	...
	if (create_blank) {
		MapSector *sector = createSector(v2size(p.X, p.Z));
...
MapBlock *ServerMap::getBlockOrEmerge(v3size p3d)
{
	MapBlock *block = getBlockNoCreateNoEx(p3d);
	if (block == NULL)
...
bool ServerMap::isBlockInQueue(v3size pos)
{
	return m_emerge && m_emerge->isBlockInQueue(pos);
}

void ServerMap::addNodeAndUpdate(v3size p, MapNode n,
		std::map<v3size, MapBlock*> &modified_blocks,
		bool remove_metadata)
{
...
void ServerMap::updateVManip(v3size pos)
{
...
void ServerMap::listAllLoadableBlocks(std::vector<v3size> &dst)
{
...
void ServerMap::listAllLoadedBlocks(std::vector<v3size> &dst)
{
	for (auto &sector_it : m_sectors) {
		MapSector *sector = sector_it.second;
...
void ServerMap::loadBlock(std::string *blob, v3size p3d, MapSector *sector, bool save_after_load)
{
	try {
...
MapBlock* ServerMap::loadBlock(v3size blockpos)
{
	bool created_new = (getBlockNoCreateNoEx(blockpos) == NULL);

	v2size p2d(blockpos.X, blockpos.Z);
...
bool ServerMap::deleteBlock(v3size blockpos)
{
...
bool ServerMap::repairBlockLight(v3size blockpos,
	std::map<v3size, MapBlock *> *modified_blocks)
{
...
void MMVManip::initialEmerge(v3size blockpos_min, v3size blockpos_max,
	bool load_if_inexistent)
{
	...
	v3size p_min = blockpos_min;
	v3size p_max = blockpos_max;

	VoxelArea block_area_nodes
			(p_min*MAP_BLOCKSIZE, (p_max+1)*MAP_BLOCKSIZE-v3size(1,1,1));
	...
	for(s32 x=p_min.X; x<=p_max.X; x++)
	{
		...
		if(block_data_inexistent)
		{
			...
			if (load_if_inexistent && !blockpos_over_max_limit(p)) {
				...
				*/
				VoxelArea a(p*MAP_BLOCKSIZE, (p+1)*MAP_BLOCKSIZE-v3size(1,1,1));
...
void MMVManip::blitBackAll(std::map<v3size, MapBlock*> *modified_blocks,
	bool overwrite_generated)
{
	if(m_area.getExtent() == v3size(0,0,0))
		return;
	assert(m_map);
