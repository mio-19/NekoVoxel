...
struct NearbyCollisionInfo {
	// node
	NearbyCollisionInfo(bool is_ul, int bouncy, const v3size &pos,
			const aabb3f &box) :
		is_unloaded(is_ul),
		obj(nullptr),
		bouncy(bouncy),
...
static inline void getNeighborConnectingFace(const v3size &p,
		...
		bool collideWithObjects)
{
	...
	*/
	...
	v3size min = floatToInt(minpos_f + box_0.MinEdge, BS) - v3size(1, 1, 1);
	v3size max = floatToInt(maxpos_f + box_0.MaxEdge, BS) + v3size(1, 1, 1);
