...
struct NearbyCollisionInfo {
	// node
	NearbyCollisionInfo(bool is_ul, int bouncy, const v3s32 &pos,
			const aabb3f &box) :
		is_unloaded(is_ul),
		obj(nullptr),
		bouncy(bouncy),
...
static inline f64 truncate(const f64 val, const f64 factor)
{
	return truncf(val * factor) / factor;
}

static inline v3d truncate(const v3d& vec, const f64 factor)
{
	return v3d(
		...
		const v3d &speed, f64 *dtime)
{
	...
	const f64 dtime_max = *dtime;
	f64 inner_margin;		// the distance of clipping recovery
	f64 distance;
	f64 time;
...
static inline void getNeighborConnectingFace(const v3s32 &p,
		...
		bool collideWithObjects)
{
	...
	*/
	...
	v3s32 min = floatToInt(minpos_f + box_0.MinEdge, BS) - v3s32(1, 1, 1);
	v3s32 max = floatToInt(maxpos_f + box_0.MaxEdge, BS) + v3s32(1, 1, 1);
	...
	if(collideWithObjects)
	{
		...
		if (c_env != 0) {
			...
			f64 distance = speed_f->getLength() * dtime +
