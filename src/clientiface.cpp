...
void RemoteClient::ResendBlockIfOnWire(v3size p)
{
		...
		std::vector<PrioritySortedBlockTransfer> &dest)
{
	...
	v3size center_nodepos = floatToInt(playerpos_predicted, BS);

	v3size center = getNodeBlockPos(center_nodepos);
	...
	*/
	...
	if (m_nearest_unsent_d > 0) {
		...
		for (const v3size &p : m_blocks_modified) {
	...
	const v3size cam_pos_nodes = floatToInt(camera_pos, BS);
	...
	for (d = d_start; d <= d_max; d++) {
		/*
			...
			box
		*/
		std::vector<v3size> list = FacePositionCache::getFacePositions(d);

		std::vector<v3size>::iterator li;
...
void RemoteClient::GotBlock(v3size p)
{
	if (m_blocks_sending.find(p) != m_blocks_sending.end()) {
...
void RemoteClient::SentBlock(v3size p)
{
	if (m_blocks_sending.find(p) == m_blocks_sending.end())
...
void RemoteClient::SetBlockNotSent(v3size p)
{
...
void RemoteClient::SetBlocksNotSent(std::map<v3size, MapBlock*> &blocks)
{
	m_nothing_to_send_pause_timer = 0;

	for (auto &block : blocks) {
		v3size p = block.first;
		// remove the block from sending and sent sets,
		// and mark as modified if found
		if (m_blocks_sending.erase(p) + m_blocks_sent.erase(p) > 0)
			m_blocks_modified.insert(p);
...
void ClientInterface::markBlockposAsNotSent(const v3size &pos)
{
	RecursiveMutexAutoLock clientslock(m_clients_mutex);
