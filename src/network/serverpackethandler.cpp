	...
	*pkt >> item_i;
	...
	if (playersao->isDead()) {
		...
		if (pointed.type == POINTEDTHING_NODE) {
			...
			v3size blockpos = getNodeBlockPos(pointed.node_undersurface);
	...
	*/
	if (!checkPriv(player->getName(), "interact")) {
		...
		// Placement -> above
			...
			v3size blockpos = getNodeBlockPos(pointed.node_abovesurface);
			client->SetBlockNotSent(blockpos);
		}
		return;
	}
	...
	*/
	...
	case INTERACT_START_DIGGING: {
		if (pointed.type == POINTEDTHING_NODE) {
			...
			v3size p_under = pointed.node_undersurface;
	...
	case INTERACT_DIGGING_COMPLETED: {
		...
		if (enable_anticheat && !isSingleplayer()) {
			v3size nocheat_p = playersao->getNoCheatDigPos();
		...
		v3size blockpos = getNodeBlockPos(p_under);
		RemoteClient *client = getClient(peer_id);
	...
	case INTERACT_PLACE: {
		...
		RemoteClient *client = getClient(peer_id);
		v3size blockpos = getNodeBlockPos(pointed.node_abovesurface);
		v3size blockpos2 = getNodeBlockPos(pointed.node_undersurface);
		if (had_prediction) {
			client->SetBlockNotSent(blockpos);
