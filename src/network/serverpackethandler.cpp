...
void Server::handleCommand_GotBlocks(NetworkPacket* pkt)
{
	...
	*pkt >> count;
	...
	if ((s32)pkt->getSize() < 1 + (int)count * 6) {
	...
	*pkt >> item_i;
	...
	if (playersao->isDead()) {
		...
		if (pointed.type == POINTEDTHING_NODE) {
			...
			v3s32 blockpos = getNodeBlockPos(pointed.node_undersurface);
	...
	*/
	if (!checkPriv(player->getName(), "interact")) {
		...
		// Placement -> above
			...
			v3s32 blockpos = getNodeBlockPos(pointed.node_abovesurface);
			client->SetBlockNotSent(blockpos);
		}
		return;
	}
	...
	*/
	...
	case INTERACT_START_DIGGING: {
		if (pointed.type == POINTEDTHING_NODE) {
			...
			v3s32 p_under = pointed.node_undersurface;
	...
	case INTERACT_DIGGING_COMPLETED: {
		...
		if (enable_anticheat && !isSingleplayer()) {
			v3s32 nocheat_p = playersao->getNoCheatDigPos();
		...
		v3s32 blockpos = getNodeBlockPos(p_under);
		RemoteClient *client = getClient(peer_id);
	...
	case INTERACT_PLACE: {
		...
		RemoteClient *client = getClient(peer_id);
		v3s32 blockpos = getNodeBlockPos(pointed.node_abovesurface);
		v3s32 blockpos2 = getNodeBlockPos(pointed.node_undersurface);
		if (had_prediction) {
			client->SetBlockNotSent(blockpos);
