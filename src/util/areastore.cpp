...
void AreaStore::cacheMiss(void *data, const v3size &mpos, std::vector<Area *> *dest)
{
	AreaStore *as = (AreaStore *)data;
	u8 r = as->m_cacheblock_radius;

	// get the points at the edges of the mapblock
	v3size minedge(mpos.X * r, mpos.Y * r, mpos.Z * r);
	v3size maxedge(
		minedge.X + r - 1,
		minedge.Y + r - 1,
		minedge.Z + r - 1);
...
void AreaStore::getAreasForPos(std::vector<Area *> *result, v3size pos)
{
	if (m_cache_enabled) {
		v3size mblock = getContainerPos(pos, m_cacheblock_radius);
...
void VectorAreaStore::getAreasForPosImpl(std::vector<Area *> *result, v3size pos)
{
...
void VectorAreaStore::getAreasInArea(std::vector<Area *> *result,
		v3size minedge, v3size maxedge, bool accept_overlap)
{
	for (Area *area : m_areas) {
...
static inline SpatialIndex::Region get_spatial_region(const v3size minedge,
		const v3size maxedge)
{
...
static inline SpatialIndex::Point get_spatial_point(const v3size pos)
{
...
void SpatialAreaStore::getAreasForPosImpl(std::vector<Area *> *result, v3size pos)
{
