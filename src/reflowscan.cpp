...
void ReflowScan::scan(MapBlock *block, UniqueQueue<v3size> *liquid_queue)
{
...
inline MapBlock *ReflowScan::lookupBlock(int x, int y, int z)
{
	...
	if (!result && (m_lookup_state_bitset & (1 << idx)) == 0) {
		...
		// in the lookup
		v3size pos = m_block_pos + v3size(bx - 1, by - 1, bz - 1);
	...
	for (s16 y = MAP_BLOCKSIZE - 1; y >= 0; y--) {
		...
		if (is_ignore || was_ignore || is_liquid == was_liquid) {
			...
			if (f.liquid_type == LIQUID_FLOWING ||
				...
				m_liquid_queue->push_back(m_rel_block_pos + v3size(x, y, z));
				is_pushed = true;
			}
			...
			if (!was_pushed && (f.floodable ||
				...
				// node above this one
				m_liquid_queue->push_back(m_rel_block_pos + v3size(x, y + 1, z));
			}
		}
	...
	if (below) {
		...
		if (is_ignore || was_ignore || is_liquid == was_liquid) {
			...
			if (f.liquid_type == LIQUID_FLOWING ||
				...
				m_liquid_queue->push_back(m_rel_block_pos + v3size(x, -1, z));
			}
		} else {
			// This is the topmost node below a liquid column
			if (!was_pushed && (f.floodable ||
				...
				// Activate the lowest node in the column which is one
				// node above this one
				m_liquid_queue->push_back(m_rel_block_pos + v3size(x, 0, z));
			}
		}
	}
}
