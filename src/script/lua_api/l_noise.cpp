...
LuaPerlinNoiseMap::LuaPerlinNoiseMap(const NoiseParams *np, s32 seed, v3size size)
{
	try {
...
int LuaPerlinNoiseMap::l_get_map_slice(lua_State *L)
{
	...
	LuaPerlinNoiseMap *o = checkObject<LuaPerlinNoiseMap>(L, 1);
	v3size slice_offset   = read_v3size(L, 2);
	v3size slice_size     = read_v3size(L, 3);
...
void *LuaPerlinNoiseMap::packIn(lua_State *L, int idx)
{
	...
	ret->size = v3size(o->noise->sx, o->noise->sy, o->noise->sz);
	return ret;
}
