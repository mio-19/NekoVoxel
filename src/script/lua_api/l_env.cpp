...
void LuaABM::trigger(ServerEnvironment *env, v3s32 p, MapNode n,
...
void LuaLBM::trigger(ServerEnvironment *env, v3s32 p,
...
int LuaRaycast::create_object(lua_State *L)
{
	...
	v3d pos1 = checkFloatPos(L, 1);
	v3d pos2 = checkFloatPos(L, 2);
...
void LuaEmergeAreaCallback(v3s32 blockpos, EmergeAction action, void *param)
{
...
int ModApiEnvMod::l_bulk_set_node(lua_State *L)
{
	...
	for (s32 i = 1; i <= len; i++) {
		lua_rawgeti(L, 1, i);
		if (!env->setNode(read_v3s32(L, -1), n))
			succeeded = false;
		lua_pop(L, 1);
	}
...
int ModApiEnvMod::l_place_node(lua_State *L)
{
	...
	pointed.node_undersurface = pos + v3s32(0,-1,0);
...
int ModApiEnvMod::l_get_objects_in_area(lua_State *L)
{
	...
	v3d minp = read_v3f(L, 1) * BS;
	v3d maxp = read_v3f(L, 2) * BS;
...
int ModApiEnvMod::l_find_node_near(lua_State *L)
{
	...
	for (int d = start_radius; d <= radius; d++) {
		const std::vector<v3s32> &list = FacePositionCache::getFacePositions(d);
		for (const v3s32 &i : list) {
			v3s32 p = pos + i;
...
static void checkArea(v3s32 &minp, v3s32 &maxp)
{
...
#define CLAMP(arg) core::clamp(arg, (s32)-MAX_MAP_GENERATION_LIMIT, (s32)MAX_MAP_GENERATION_LIMIT)
	minp = v3s32(CLAMP(minp.X), CLAMP(minp.Y), CLAMP(minp.Z));
	maxp = v3s32(CLAMP(maxp.X), CLAMP(maxp.Y), CLAMP(maxp.Z));
#undef CLAMP
}
...
int ModApiEnvMod::l_find_nodes_in_area(lua_State *L)
{
	...
	v3s32 minp = read_v3s32(L, 1);
	v3s32 maxp = read_v3s32(L, 2);
	...
	if (grouped) {
		...
		map.forEachNodeInArea(minp, maxp, [&](v3s32 p, MapNode n) -> bool {
...
int ModApiEnvMod::l_get_perlin(lua_State *L)
{
	...
	if (lua_istable(L, 1)) {
		...
		params.spread  = v3d(1, 1, 1) * readParam<float>(L, 4);
	}
...
int ModApiEnvMod::l_get_voxel_manip(lua_State *L)
{
	...
	LuaVoxelManip *o = (lua_istable(L, 1) && lua_istable(L, 2)) ?
		new LuaVoxelManip(map, read_v3s32(L, 1), read_v3s32(L, 2)) :
...
int ModApiEnvMod::l_fix_light(lua_State *L)
{
	...
	v3s32 blockpos1 = getContainerPos(read_v3s32(L, 1), MAP_BLOCKSIZE);
	v3s32 blockpos2 = getContainerPos(read_v3s32(L, 2), MAP_BLOCKSIZE);
...
int ModApiEnvMod::l_load_area(lua_State *L)
{
	...
	if (!lua_istable(L, 2)) {
		...
		for (s32 x = bp1.X; x <= bp2.X; x++) {
			map->emergeBlock(v3s32(x, y, z));
		}
	}
...
int ModApiEnvMod::l_emerge_area(lua_State *L)
{
	...
	v3s32 bpmin = getNodeBlockPos(read_v3s32(L, 1));
	v3s32 bpmax = getNodeBlockPos(read_v3s32(L, 2));
	...
	for (s32 x = bpmin.X; x <= bpmax.X; x++) {
		emerge->enqueueBlockEmergeEx(v3s32(x, y, z), PEER_ID_INEXISTENT,
...
int ModApiEnvMod::l_find_path(lua_State *L)
{
	...
	v3s32 pos1                  = read_v3s32(L, 1);
	v3s32 pos2                  = read_v3s32(L, 2);
	...
	std::vector<v3s32> path = get_path(&env->getServerMap(), env->getGameDef()->ndef(), pos1, pos2,
