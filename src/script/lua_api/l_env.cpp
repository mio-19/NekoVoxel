...
void LuaABM::trigger(ServerEnvironment *env, v3size p, MapNode n,
...
void LuaLBM::trigger(ServerEnvironment *env, v3size p,
...
void LuaEmergeAreaCallback(v3size blockpos, EmergeAction action, void *param)
{
...
int ModApiEnvMod::l_bulk_set_node(lua_State *L)
{
	...
	for (s32 i = 1; i <= len; i++) {
		lua_rawgeti(L, 1, i);
		if (!env->setNode(read_v3size(L, -1), n))
			succeeded = false;
		lua_pop(L, 1);
	}
...
int ModApiEnvMod::l_place_node(lua_State *L)
{
	...
	pointed.node_undersurface = pos + v3size(0,-1,0);
...
int ModApiEnvMod::l_find_nodes_with_meta(lua_State *L)
{
	...
	std::vector<v3size> positions = env->getMap().findNodesWithMetadata(
		check_v3size(L, 1), check_v3size(L, 2));

	lua_createtable(L, positions.size(), 0);
	for (size_t i = 0; i != positions.size(); i++) {
		push_v3size(L, positions[i]);
		lua_rawseti(L, -2, i + 1);
	}
...
int ModApiEnvMod::l_find_node_near(lua_State *L)
{
	...
	for (int d = start_radius; d <= radius; d++) {
		const std::vector<v3size> &list = FacePositionCache::getFacePositions(d);
		for (const v3size &i : list) {
			v3size p = pos + i;
...
static void checkArea(v3size &minp, v3size &maxp)
{
...
int ModApiEnvMod::l_find_nodes_in_area(lua_State *L)
{
	...
	v3size minp = read_v3size(L, 1);
	v3size maxp = read_v3size(L, 2);
	...
	if (grouped) {
		...
		map.forEachNodeInArea(minp, maxp, [&](v3size p, MapNode n) -> bool {
...
int ModApiEnvMod::l_get_voxel_manip(lua_State *L)
{
	...
	LuaVoxelManip *o = (lua_istable(L, 1) && lua_istable(L, 2)) ?
		new LuaVoxelManip(map, read_v3size(L, 1), read_v3size(L, 2)) :
...
int ModApiEnvMod::l_fix_light(lua_State *L)
{
	...
	v3size blockpos1 = getContainerPos(read_v3size(L, 1), MAP_BLOCKSIZE);
	v3size blockpos2 = getContainerPos(read_v3size(L, 2), MAP_BLOCKSIZE);
...
int ModApiEnvMod::l_load_area(lua_State *L)
{
	...
	Map *map = &(env->getMap());
	v3size bp1 = getNodeBlockPos(check_v3size(L, 1));
	if (!lua_istable(L, 2)) {
		map->emergeBlock(bp1);
	} else {
		v3size bp2 = getNodeBlockPos(check_v3size(L, 2));
		sortBoxVerticies(bp1, bp2);
		for (s16 z = bp1.Z; z <= bp2.Z; z++)
		for (s16 y = bp1.Y; y <= bp2.Y; y++)
		for (s16 x = bp1.X; x <= bp2.X; x++) {
			map->emergeBlock(v3size(x, y, z));
		}
	}
...
int ModApiEnvMod::l_emerge_area(lua_State *L)
{
	...
	v3size bpmin = getNodeBlockPos(read_v3size(L, 1));
	v3size bpmax = getNodeBlockPos(read_v3size(L, 2));
	...
	for (s16 x = bpmin.X; x <= bpmax.X; x++) {
		emerge->enqueueBlockEmergeEx(v3size(x, y, z), PEER_ID_INEXISTENT,
...
int ModApiEnvMod::l_find_path(lua_State *L)
{
	...
	v3size pos1                  = read_v3size(L, 1);
	v3size pos2                  = read_v3size(L, 2);
	...
	std::vector<v3size> path = get_path(&env->getServerMap(), env->getGameDef()->ndef(), pos1, pos2,
...
int ModApiEnvMod::l_forceload_block(lua_State *L)
{
	...
	v3size blockpos = read_v3size(L, 1);
...
int ModApiEnvMod::l_compare_block_status(lua_State *L)
{
	...
	v3size nodepos = check_v3size(L, 1);
