...
public:
	...
	void environment_OnGenerated(v3size minp, v3size maxp, u32 blockseed);
	...
	void on_emerge_area_completion(v3size blockpos, int action,
		ScriptCallbackState *state);

	void check_for_falling(v3size p);

	// Called after liquid transform changes
	void on_liquid_transformed(const std::vector<std::pair<v3size, MapNode>> &list);

	// Called after mapblock changes
	void on_mapblocks_changed(const std::unordered_set<v3size> &set);
