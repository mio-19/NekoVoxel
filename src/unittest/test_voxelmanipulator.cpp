...
{
	VoxelArea a(v3s32(-1,-1,-1), v3s32(1,1,1));
	UASSERT(a.index(0,0,0) == 1*3*3 + 1*3 + 1);
	UASSERT(a.index(-1,-1,-1) == 0);

	VoxelArea c(v3s32(-2,-2,-2), v3s32(2,2,2));
	// An area that is 1 bigger in x+ and z-
	VoxelArea d(v3s32(-2,-2,-3), v3s32(3,2,2));
	...
	results.emplace_back(v3s32(-2,-2,-3), v3s32(3,2,-3));
	results.emplace_back(v3s32(3,-2,-2), v3s32(3,2,2));
...
void TestVoxelManipulator::testVoxelManipulator(const NodeDefManager *nodedef)
{
	...
	v.setNodeNoRef(v3s32(-1,0,-1), MapNode(t_CONTENT_GRASS));

	v.print(infostream, nodedef);
	UASSERT(v.getNode(v3s32(-1,0,-1)).getContent() == t_CONTENT_GRASS);

	infostream << "*** Reading from inexistent (0,0,-1) ***" << std::endl;

	EXCEPTION_CHECK(InvalidPositionException, v.getNode(v3s32(0,0,-1)));
	v.print(infostream, nodedef);
	...
	UASSERT(v.getNode(v3s32(-1,0,-1)).getContent() == t_CONTENT_GRASS);
	EXCEPTION_CHECK(InvalidPositionException, v.getNode(v3s32(0,1,1)));
}
