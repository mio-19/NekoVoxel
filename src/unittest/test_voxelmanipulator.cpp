...
{
	VoxelArea a(v3size(-1,-1,-1), v3size(1,1,1));
	UASSERT(a.index(0,0,0) == 1*3*3 + 1*3 + 1);
	UASSERT(a.index(-1,-1,-1) == 0);

	VoxelArea c(v3size(-2,-2,-2), v3size(2,2,2));
	// An area that is 1 bigger in x+ and z-
	VoxelArea d(v3size(-2,-2,-3), v3size(3,2,2));
	...
	results.emplace_back(v3size(-2,-2,-3), v3size(3,2,-3));
	results.emplace_back(v3size(3,-2,-2), v3size(3,2,2));
...
void TestVoxelManipulator::testVoxelManipulator(const NodeDefManager *nodedef)
{
	...
	v.setNodeNoRef(v3size(-1,0,-1), MapNode(t_CONTENT_GRASS));

	v.print(infostream, nodedef);
	UASSERT(v.getNode(v3size(-1,0,-1)).getContent() == t_CONTENT_GRASS);

	infostream << "*** Reading from inexistent (0,0,-1) ***" << std::endl;

	EXCEPTION_CHECK(InvalidPositionException, v.getNode(v3size(0,0,-1)));
	v.print(infostream, nodedef);
	...
	UASSERT(v.getNode(v3size(-1,0,-1)).getContent() == t_CONTENT_GRASS);
	EXCEPTION_CHECK(InvalidPositionException, v.getNode(v3size(0,1,1)));
}
