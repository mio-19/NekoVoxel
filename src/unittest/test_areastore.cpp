...
{
	Area a(v3s32(-10, -3, 5), v3s32(0, 29, 7));
	Area b(v3s32(-5, -2, 5), v3s32(0, 28, 6));
	Area c(v3s32(-7, -3, 6), v3s32(-1, 27, 7));
	std::vector<Area *> res;
	...
	store->getAreasForPos(&res, v3s32(-1, 0, 6));
	UASSERTEQ(size_t, res.size(), 3);
	res.clear();
	store->getAreasForPos(&res, v3s32(0, 0, 7));
	...
	store->getAreasInArea(&res, v3s32(-10, -3, 5), v3s32(0, 29, 7), false);
	UASSERTEQ(size_t, res.size(), 3);
	res.clear();

	store->getAreasInArea(&res, v3s32(-100, 0, 6), v3s32(200, 0, 6), false);
	UASSERTEQ(size_t, res.size(), 0);
	res.clear();

	store->getAreasInArea(&res, v3s32(-100, 0, 6), v3s32(200, 0, 6), true);
	UASSERTEQ(size_t, res.size(), 3);
	res.clear();
	...
	Area d(v3s32(-100, -300, -200), v3s32(-50, -200, -100));
	d.data = "Hi!";
	store->insertArea(&d);

	store->getAreasForPos(&res, v3s32(-75, -250, -150));
	UASSERTEQ(size_t, res.size(), 1);
...
void TestAreaStore::testSerialization()
{
	...
	Area a(v3s32(-1, 0, 1), v3s32(0, 1, 2));
	a.data = "Area AA";
	store.insertArea(&a);

	Area b(v3s32(123, 456, 789), v3s32(32000, 100, 10));
	b.data = "Area BB";
	store.insertArea(&b);
	...
	Area c(v3s32(33, -2, -6), v3s32(4, 77, -76));
	c.data = "Area CC";
	store.insertArea(&c);
