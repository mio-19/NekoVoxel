...
void TestMap::testMaxMapgenLimit()
{
	...
	UASSERT(blockpos_over_max_limit(v3s32(limit_block)) == false);
	UASSERT(blockpos_over_max_limit(v3s32(limit_block+1)) == true);
	UASSERT(blockpos_over_max_limit(v3s32(-limit_block)) == false);
	UASSERT(blockpos_over_max_limit(v3s32(-limit_block-1)) == true);
}
...
{
	v3s32 minp_visit(-10, -10, -10);
	v3s32 maxp_visit(20, 20, 10);
	v3s32 dims_visit = maxp_visit - minp_visit + v3s32(1, 1, 1);
	s32 volume_visit = (s32)dims_visit.X * (s32)dims_visit.Y * (s32)dims_visit.Z;

	v3s32 minp = minp_visit - v3s32(1, 1, 1);
	v3s32 maxp = maxp_visit + v3s32(1, 1, 1);
	...
	s32 n_visited = 0;
	std::unordered_set<v3s32> visited;
	v3s32 minp_visited(0, 0, 0);
	v3s32 maxp_visited(0, 0, 0);
	std::unordered_map<v3s32, MapNode> found;
	map.forEachNodeInArea(minp_visit, maxp_visit, [&](v3s32 p, MapNode n) -> bool {
		n_visited++;
		visited.insert(p);
...
{
	DummyMap map(gamedef, v3s32(0, 0, 0), v3s32(-1, -1, -1));

	v3s32 invalid_p(0, 0, 0);
	bool visited = false;
	map.forEachNodeInArea(invalid_p, invalid_p, [&](v3s32 p, MapNode n) -> bool {
...
{
	DummyMap map(gamedef, v3s32(), v3s32());
	map.forEachNodeInArea(v3s32(0, 0, 0), v3s32(-1, -1, -1), [&](v3s32 p, MapNode n) -> bool {
