...
void TestMap::testMaxMapgenLimit()
{
	...
	UASSERT(blockpos_over_max_limit(v3size(limit_block)) == false);
	UASSERT(blockpos_over_max_limit(v3size(limit_block+1)) == true);
	UASSERT(blockpos_over_max_limit(v3size(-limit_block)) == false);
	UASSERT(blockpos_over_max_limit(v3size(-limit_block-1)) == true);
}
...
{
	v3size minp_visit(-10, -10, -10);
	v3size maxp_visit(20, 20, 10);
	v3size dims_visit = maxp_visit - minp_visit + v3size(1, 1, 1);
	s32 volume_visit = (s32)dims_visit.X * (s32)dims_visit.Y * (s32)dims_visit.Z;

	v3size minp = minp_visit - v3size(1, 1, 1);
	v3size maxp = maxp_visit + v3size(1, 1, 1);
	...
	v3size p3 = minp_visit;
	MapNode n3(CONTENT_AIR);
	map.setNode(p3, n3);

	v3size p4 = maxp_visit;
	...
	s32 n_visited = 0;
	std::unordered_set<v3size> visited;
	v3size minp_visited(0, 0, 0);
	v3size maxp_visited(0, 0, 0);
	std::unordered_map<v3size, MapNode> found;
	map.forEachNodeInArea(minp_visit, maxp_visit, [&](v3size p, MapNode n) -> bool {
		n_visited++;
		visited.insert(p);
...
{
	DummyMap map(gamedef, v3size(0, 0, 0), v3size(-1, -1, -1));

	v3size invalid_p(0, 0, 0);
	bool visited = false;
	map.forEachNodeInArea(invalid_p, invalid_p, [&](v3size p, MapNode n) -> bool {
...
{
	DummyMap map(gamedef, v3size(), v3size());
	map.forEachNodeInArea(v3size(0, 0, 0), v3size(-1, -1, -1), [&](v3size p, MapNode n) -> bool {
