...
struct TestMapBlock: public TestBase
{
	...
	public:
		...
		MapNode node;
		bool position_valid;
		std::list<v3s32> validity_exceptions;
		...
		virtual bool isValidPosition(v3s32 p)
		{
			//return position_valid ^ (p==position_valid_exception);
			bool exception = false;
			for(std::list<v3s32>::iterator i=validity_exceptions.begin();
		...
		virtual MapNode getNode(v3s32 p)
		{
		...
		virtual void setNode(v3s32 p, MapNode & n)
		{
			if(isValidPosition(p) == false)
				throw InvalidPositionException();
		};
	...
	void Run()
	{
		...
		MapBlock b(&parent, v3s32(1,1,1));
		v3s32 relpos(MAP_BLOCKSIZE, MAP_BLOCKSIZE, MAP_BLOCKSIZE);
		...
		UASSERT(b.isValidPosition(v3s32(0,0,0)) == true);
		UASSERT(b.isValidPosition(v3s32(-1,0,0)) == false);
		UASSERT(b.isValidPosition(v3s32(-1,-142,-2341)) == false);
		UASSERT(b.isValidPosition(v3s32(-124,142,2341)) == false);
		UASSERT(b.isValidPosition(v3s32(MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1)) == true);
		UASSERT(b.isValidPosition(v3s32(MAP_BLOCKSIZE-1,MAP_BLOCKSIZE,MAP_BLOCKSIZE-1)) == false);

		/*
			TODO: this method should probably be removed
			if the block size isn't going to be set variable
		*/
		/*UASSERT(b.getSizeNodes() == v3s32(MAP_BLOCKSIZE,
		...
		{
			//UASSERT(b.getNode(v3s32(x,y,z)).getContent() == CONTENT_AIR);
			UASSERT(b.getNode(v3s32(x,y,z)).getContent() == CONTENT_IGNORE);
			UASSERT(b.getNode(v3s32(x,y,z)).getLight(LIGHTBANK_DAY) == 0);
			UASSERT(b.getNode(v3s32(x,y,z)).getLight(LIGHTBANK_NIGHT) == 0);
		}
		...
		*/
		...
		UASSERT(b.isValidPositionParent(v3s32(0,0,0)) == true);
		UASSERT(b.isValidPositionParent(v3s32(MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1)) == true);
		n = b.getNodeParent(v3s32(0,MAP_BLOCKSIZE-1,0));
		UASSERT(n.getContent() == CONTENT_AIR);

		// ...but outside the block they should be invalid
		UASSERT(b.isValidPositionParent(v3s32(-121,2341,0)) == false);
		UASSERT(b.isValidPositionParent(v3s32(-1,0,0)) == false);
		UASSERT(b.isValidPositionParent(v3s32(MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1,MAP_BLOCKSIZE)) == false);

		{
			bool exception_thrown = false;
			try{
				// This should throw an exception
				MapNode n = b.getNodeParent(v3s32(0,0,-1));
			}
		...
		UASSERT(b.isValidPositionParent(v3s32(-121,2341,0)) == true);
		UASSERT(b.isValidPositionParent(v3s32(-1,0,0)) == true);
		UASSERT(b.isValidPositionParent(v3s32(MAP_BLOCKSIZE-1,MAP_BLOCKSIZE-1,MAP_BLOCKSIZE)) == true);
		n = b.getNodeParent(v3s32(0,0,MAP_BLOCKSIZE));
		...
		*/
		...
		UASSERT(b.getNodeTile(v3s32(-1,-1,0)) == 5);*/
		...
		*/
		...
		for(u16 z=0; z<MAP_BLOCKSIZE; z++){
			for(u16 y=0; y<MAP_BLOCKSIZE; y++){
				for(u16 x=0; x<MAP_BLOCKSIZE; x++){
					MapNode n = b.getNode(v3s32(x,y,z));
		...
		}
		{
			...
			*/
			...
			std::map<v3s32, bool> light_sources;
			// The bottom block is invalid, because we have a shadowing node
			UASSERT(b.propagateSunlight(light_sources) == false);
			UASSERT(b.getNode(v3s32(1,4,0)).getLight(LIGHTBANK_DAY) == LIGHT_SUN);
			UASSERT(b.getNode(v3s32(1,3,0)).getLight(LIGHTBANK_DAY) == LIGHT_SUN);
			UASSERT(b.getNode(v3s32(1,2,0)).getLight(LIGHTBANK_DAY) == 0);
			UASSERT(b.getNode(v3s32(1,1,0)).getLight(LIGHTBANK_DAY) == 0);
			UASSERT(b.getNode(v3s32(1,0,0)).getLight(LIGHTBANK_DAY) == 0);
			UASSERT(b.getNode(v3s32(1,2,3)).getLight(LIGHTBANK_DAY) == LIGHT_SUN);
			UASSERT(b.getFaceLight2(1000, p, v3s32(0,1,0)) == LIGHT_SUN);
			UASSERT(b.getFaceLight2(1000, p, v3s32(0,-1,0)) == 0);
			UASSERT(b.getFaceLight2(0, p, v3s32(0,-1,0)) == 0);
			// According to MapBlock::getFaceLight,
			// The face on the z+ side should have double-diminished light
			//UASSERT(b.getFaceLight(p, v3s32(0,0,1)) == diminish_light(diminish_light(LIGHT_MAX)));
			// The face on the z+ side should have diminished light
			UASSERT(b.getFaceLight2(1000, p, v3s32(0,0,1)) == diminish_light(LIGHT_MAX));
		}
		/*
		...
		*/
		{
			...
			//UASSERT(b.getNode(v3s32(1,2,3)).getLight() == LIGHT_SUN);
			// Should set light of non-sunlighted blocks to 0.
			UASSERT(b.getNode(v3s32(1,2,3)).getLight(LIGHTBANK_DAY) == 0);
		}
		/*
		...
		*/
		{
			...
			for(u16 z=0; z<MAP_BLOCKSIZE; z++)
			{
				parent.validity_exceptions.push_back(v3s32(MAP_BLOCKSIZE+x, MAP_BLOCKSIZE-1, MAP_BLOCKSIZE+z));
			}
...
struct TestMapSector: public TestBase
{
	...
	void Run()
	{
		...
		ServerMapSector sector(&parent, v2s32(1,1));
