...
public:
	...
	*/
	...
	void sendNodemetaFields(v3size p, const std::string &formname,
	...
	void removeNode(v3size p);

	// helpers to enforce CSM restrictions
	MapNode CSMGetNode(v3size p, bool *is_valid_position);
	int CSMClampRadius(v3size pos, int radius);
	v3size CSMClampPos(v3size pos);

	void addNode(v3size p, MapNode n, bool remove_metadata = true);
	...
	int getCrackLevel();
	v3size getCrackPos();
	void setCrack(int level, v3size pos);
	...
	void addUpdateMeshTask(v3size blockpos, bool ack_to_server=false, bool urgent=false);
	// Including blocks at appropriate edges
	void addUpdateMeshTaskWithEdge(v3size blockpos, bool ack_to_server=false, bool urgent=false);
	void addUpdateMeshTaskForNode(v3size nodepos, bool ack_to_server=false, bool urgent=false);

	void updateCameraOffset(v3size camera_offset)
...
private:
	...
	void sendDeletedBlocks(std::vector<v3size> &blocks);
	void sendGotBlocks(const std::vector<v3size> &blocks);
