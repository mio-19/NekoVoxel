...
struct MinimapMapblock {
	void getMinimapNodes(VoxelManipulator *vmanip, const v3size &pos);
...
public:
	...
	void getMap(v3size pos, s16 size, s16 height);
	void enqueueBlock(v3size pos, MinimapMapblock *data);
	bool pushBlockUpdate(v3size pos, MinimapMapblock *data);
...
private:
	...
	std::map<v3size, MinimapMapblock *> m_blocks_cache;
};
...
public:
	...
	void addBlock(v3size pos, MinimapMapblock *data);

	v3f getYawVec();

	void setPos(v3size pos);
	v3size getPos() const { return data->pos; }
