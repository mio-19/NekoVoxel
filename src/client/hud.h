...
public:
	...
	void updateSelectionMesh(const v3size &camera_offset);

	std::vector<aabb3f> *getSelectionBoxes() { return &m_selection_boxes; }

	void setSelectionPos(const v3f &pos, const v3size &camera_offset);
	...
	void drawLuaElements(const v3size &camera_offset);

private:
	bool calculateScreenPos(const v3size &camera_offset, HudElement *e, v2s32 *pos);
