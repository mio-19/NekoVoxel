...
void Client::step(float dtime)
{
	...
	*/
	...
	if(m_map_timer_and_unload_interval.step(dtime, map_timer_and_unload_dtime)) {
		std::vector<v3size> deleted_blocks;
		...
		*/
		...
		std::vector<v3size>::iterator i = deleted_blocks.begin();
		std::vector<v3size> sendlist;
		for(;;) {
	...
	*/
	{
		...
		std::vector<v3size> blocks_to_ack;
...
void Client::sendDeletedBlocks(std::vector<v3size> &blocks)
{
	NetworkPacket pkt(TOSERVER_DELETEDBLOCKS, 1 + sizeof(v3size) * blocks.size());

	pkt << (u8) blocks.size();

	for (const v3size &block : blocks) {
		pkt << block;
	}
...
void Client::sendGotBlocks(const std::vector<v3size> &blocks)
{
...
void Client::sendNodemetaFields(v3size p, const std::string &formname,
...
void Client::removeNode(v3size p)
{
 ...
 * @return
 */
MapNode Client::CSMGetNode(v3size p, bool *is_valid_position)
{
	if (checkCSMRestrictionFlag(CSMRestrictionFlags::CSM_RF_LOOKUP_NODES)) {
		v3size ppos = floatToInt(m_env.getLocalPlayer()->getPosition(), BS);
...
int Client::CSMClampRadius(v3size pos, int radius)
{
	if (!checkCSMRestrictionFlag(CSMRestrictionFlags::CSM_RF_LOOKUP_NODES))
		return radius;
...
v3size Client::CSMClampPos(v3size pos)
{
	if (!checkCSMRestrictionFlag(CSMRestrictionFlags::CSM_RF_LOOKUP_NODES))
		return pos;
	v3size ppos = floatToInt(m_env.getLocalPlayer()->getPosition(), BS);
...
void Client::addNode(v3size p, MapNode n, bool remove_metadata)
{
...
v3size Client::getCrackPos()
{
	return m_crack_pos;
}

void Client::setCrack(int level, v3size pos)
{
	int old_crack_level = m_crack_level;
	v3size old_crack_pos = m_crack_pos;
...
void Client::addUpdateMeshTask(v3size p, bool ack_to_server, bool urgent)
{
...
void Client::addUpdateMeshTaskWithEdge(v3size blockpos, bool ack_to_server, bool urgent)
{
	m_mesh_update_manager.updateBlock(&m_env.getMap(), blockpos, ack_to_server, urgent, true);
}

void Client::addUpdateMeshTaskForNode(v3size nodepos, bool ack_to_server, bool urgent)
{
	{
		v3size p = nodepos;
	...
	v3size blockpos = getNodeBlockPos(nodepos);
	v3size blockpos_relative = blockpos * MAP_BLOCKSIZE;
	m_mesh_update_manager.updateBlock(&m_env.getMap(), blockpos, ack_to_server, urgent, false);
	// Leading edge
	if (nodepos.X == blockpos_relative.X)
		addUpdateMeshTask(blockpos + v3size(-1, 0, 0), false, urgent);
	if (nodepos.Y == blockpos_relative.Y)
		addUpdateMeshTask(blockpos + v3size(0, -1, 0), false, urgent);
	if (nodepos.Z == blockpos_relative.Z)
		addUpdateMeshTask(blockpos + v3size(0, 0, -1), false, urgent);
}
