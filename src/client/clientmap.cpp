...
void MeshBufListList::add(scene::IMeshBuffer *buf, v3size position, u8 layer)
{
...
ClientMap::ClientMap(
	...
	m_control(control),
	m_drawlist(MapBlockComparer(v3size(0,0,0)))
{
...
void ClientMap::updateCamera(v3f pos, v3f dir, f32 fov, v3size offset)
{
	v3size previous_node = floatToInt(m_camera_position, BS) + m_camera_offset;
	v3size previous_block = getContainerPos(previous_node, MAP_BLOCKSIZE);
	...
	v3size current_node = floatToInt(m_camera_position, BS) + m_camera_offset;
	v3size current_block = getContainerPos(current_node, MAP_BLOCKSIZE);
...
MapSector * ClientMap::emergeSector(v2size p2d)
{
...
void ClientMap::getBlocksInViewRange(v3size cam_pos_nodes,
		v3size *p_blocks_min, v3size *p_blocks_max, float range)
{
	if (range <= 0.0f)
		range = m_control.wanted_range;

	v3size box_nodes_d = range * v3size(1, 1, 1);
	// Define p_nodes_min/max as v3s32 because 'cam_pos_nodes -/+ box_nodes_d'
	// can exceed the range of v3size when a large view range is used near the
	// world edges.
	v3s32 p_nodes_min(
...
public:
	...
	MapBlockFlags(v3size min_pos, v3size max_pos)
	...
	private:
		inline std::size_t getAddress(v3size pos) {
	...
	Chunk &getChunk(v3size pos)
	{
		v3size delta = (pos - min_pos) / CHUNK_EDGE;
...
void ClientMap::updateDrawList()
{
	if (m_new_occlusion_culler) {
		...
		v3size cam_pos_nodes = floatToInt(m_camera_position, BS);
		...
		v3size camera_block = getContainerPos(cam_pos_nodes, MAP_BLOCKSIZE);
		m_drawlist = std::map<v3size, MapBlock*, MapBlockComparer>(MapBlockComparer(camera_block));
		...
		std::queue<v3size> blocks_to_consider;
		...
		while (blocks_to_consider.size() > 0) {
			...
			v3size block_coord = blocks_to_consider.front();
			...
			MapSector *sector = this->getSectorNoGenerate(v2size(block_coord.X, block_coord.Z));
			...
			v3size block_pos_nodes = block_coord * MAP_BLOCKSIZE;
			...
			v3size look = block_coord - camera_block;
			...
			v3size precise_look = 2 * (block_pos_nodes - cam_pos_nodes) + MAP_BLOCKSIZE - 1;
			...
			*/
			...
			for (MapBlock *block : sectorblocks) {
				...
				*/
				...
				v3size block_coord = block->getPos();
	...
	int *result, bool *sunlight_seen)
{
	...
	{
		v3size p = floatToInt(p0 /*+ dir * 3*BS*/, BS);
...
void ClientMap::updateDrawListShadow(v3f shadow_light_pos, v3f shadow_light_dir, float radius, float length)
{
	...
	v3size cam_pos_nodes = floatToInt(shadow_light_pos, BS);
	v3size p_blocks_min;
	v3size p_blocks_max;
