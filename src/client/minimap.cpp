...
bool MinimapUpdateThread::pushBlockUpdate(v3size pos, MinimapMapblock *data)
{
...
void MinimapUpdateThread::enqueueBlock(v3size pos, MinimapMapblock *data)
{
...
void MinimapUpdateThread::doUpdate()
{
	...
	while (popBlockUpdate(&update)) {
		if (update.data) {
			...
			std::pair<std::map<v3size, MinimapMapblock*>::iterator, bool>
			...
			std::map<v3size, MinimapMapblock *>::iterator it;
...
void MinimapUpdateThread::getMap(v3size pos, s16 size, s16 height)
{
	v3size pos_min(pos.X - size / 2, pos.Y - height / 2, pos.Z - size / 2);
	v3size pos_max(pos_min.X + size - 1, pos.Y + height / 2, pos_min.Z + size - 1);
	v3size blockpos_min = getNodeBlockPos(pos_min);
	v3size blockpos_max = getNodeBlockPos(pos_max);
...
// draw the map
	...
	for (blockpos.X = blockpos_min.X; blockpos.X <= blockpos_max.X; ++blockpos.X) {
		std::map<v3size, MinimapMapblock *>::const_iterator pblock =
			m_blocks_cache.find(blockpos);
		if (pblock == m_blocks_cache.end())
			continue;
		const MinimapMapblock &block = *pblock->second;

		v3size block_node_min(blockpos * MAP_BLOCKSIZE);
		v3size block_node_max(block_node_min + MAP_BLOCKSIZE - 1);
		// clip
		v3size range_min = componentwise_max(block_node_min, pos_min);
		v3size range_max = componentwise_min(block_node_max, pos_max);

		v3size pos;
		pos.Y = range_min.Y;
		for (pos.Z = range_min.Z; pos.Z <= range_max.Z; ++pos.Z)
		for (pos.X = range_min.X; pos.X <= range_max.X; ++pos.X) {
			v3size inblock_pos = pos - block_node_min;
			const MinimapPixel &in_pixel =
				block.data[inblock_pos.Z * MAP_BLOCKSIZE + inblock_pos.X];

			v3size inmap_pos = pos - pos_min;
...
void Minimap::addBlock(v3size pos, MinimapMapblock *data)
{
...
void Minimap::setPos(v3size pos)
{
...
void Minimap::updateActiveMarkers()
{
	...
	v3size pos_offset = data->pos - v3size(data->mode.map_size / 2,
			data->mode.scan_height / 2,
			data->mode.map_size / 2);

	for (MinimapMarker *marker : m_markers) {
		v3size pos = floatToInt(marker->parent_node->getAbsolutePosition() +
...
void MinimapMapblock::getMinimapNodes(VoxelManipulator *vmanip, const v3size &pos)
{
