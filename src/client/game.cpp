...
{
	TextDestNodeMetadata(v3s32 p, Client *client)
	{
		m_p = p;
		m_client = client;
	}
...
{
public:
	NodeMetadataFormSource(ClientMap *map, v3s32 p):
		m_map(map),
		m_p(p)
	{
	}
...
public:
	...
	NodeDugEvent(v3s32 p, MapNode n):
		p(p),
		n(n)
	{}
...
{
	s32 range = g_settings->getS32("viewing_range");
	s32 range_new = range + 10;
...
void Game::updateCamera(f64 dtime)
{
	...
	*/
	...
	v3s32 old_camera_offset = camera->getOffset();
...
bool Game::nodePlacement(const ItemDefinition &selected_def,
	const ItemStack &selected_item, const v3s32 &nodepos, const v3s32 &neighborpos,
	const PointedThing &pointed, const NodeMetadata *meta)
{
	...
	if (place_param2) {
		...
		v3s32 dir = nodepos - floatToInt(client->getEnv().getLocalPlayer()->getPosition(), BS);
	...
	try {
		...
		if (!predicted_f.walkable ||
				...
				(predicted_f.walkable &&
					neighborpos != player->getStandingNodePos() + v3s32(0, 1, 0) &&
					neighborpos != player->getStandingNodePos() + v3s32(0, 2, 0))) {
