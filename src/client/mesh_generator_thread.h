...
{
	v3size p = v3size(-1337, -1337, -1337);
...
public:
	...
	bool addBlock(Map *map, v3size p, bool ack_block_to_server, bool urgent);
...
private:
	...
	std::unordered_set<v3size> m_urgents;
	std::unordered_set<v3size> m_inflight_blocks;
	std::mutex m_mutex;
...
{
	v3size p = v3size(-1338, -1338, -1338);
...
{
public:
	MeshUpdateWorkerThread(MeshUpdateQueue *queue_in, MeshUpdateManager *manager, v3size *camera_offset);
...
public:
	...
	// Caches the block at p and its neighbors (if needed) and queues a mesh
	// update for the block at p
	void updateBlock(Map *map, v3size p, bool ack_block_to_server, bool urgent,
