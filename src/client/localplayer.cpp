	...
	const v3f &sneak_max)
{
	...
	static const v3size dir9_center[9] = {
		v3size( 0, 0,  0),
		v3size( 1, 0,  0),
		v3size(-1, 0,  0),
		v3size( 0, 0,  1),
	...
	const v3size current_node = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
	...
	for (const auto &d : dir9_center) {
		...
		if (!physics_override.sneak_glitch) {
			...
			for (u16 y = 1; y <= height; y++) {
				node = map->getNode(p + v3size(0, y, 0), &is_valid_position);
				if (!is_valid_position || nodemgr->get(node).walkable) {
					ok = false;
					break;
				}
			...
			node = map->getNode(p + v3size(0, 1, 0), &is_valid_position);
	...
	if (physics_override.sneak_glitch) {
		...
		node = map->getNode(m_sneak_node + v3size(0, 2, 0),
			&is_valid_position);
		if (is_valid_position && nodemgr->get(node).walkable) {
			// Node three meters above: must be non-solid
			node = map->getNode(m_sneak_node + v3size(0, 3, 0),
				&is_valid_position);
		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	v3size pp2 = floatToInt(position + v3f(0.0f, -0.2f * BS, 0.0f), BS);
	node = map->getNode(pp, &is_valid_position);
	...
	*/
	...
	const ContentFeatures &f1 = nodemgr->get(map->getNode(m_standing_node + v3size(0, 1, 0)));
...
v3size LocalPlayer::getStandingNodePos()
{
	if (m_sneak_node_exists)
		return m_sneak_node;
...
v3size LocalPlayer::getFootstepNodePos()
{
...
v3size LocalPlayer::getLightPosition() const
{
	...
	std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	v3size current_node = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
	if (m_sneak_node_exists &&
	...
	if (m_need_to_get_new_sneak_node && physics_override.sneak) {
		...
		v3size pos_i_bottom = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
		v2f player_p2df(position.X, position.Z);
		f32 min_distance_f = 100000.0f * BS;
		// If already seeking from some node, compare to it.
		v3size new_sneak_node = m_sneak_node;
		for (s16 x= -1; x <= 1; x++)
		for (s16 z= -1; z <= 1; z++) {
			v3size p = pos_i_bottom + v3size(x, 0, z);
			v3f pf = intToFloat(p, BS);
			...
			if (!physics_override.sneak_glitch) {
				node = map->getNode(p + v3size(0, 2, 0), &is_valid_position);
				if (!is_valid_position || nodemgr->get(node).walkable)
					continue;
			}
	...
	const v3f &initial_speed, f32 pos_max_d)
{
	...
	v3size ceilpos_min = floatToInt(headpos_min, BS) + v3size(0, 1, 0);
	v3size ceilpos_max = floatToInt(headpos_max, BS) + v3size(0, 1, 0);
	const NodeDefManager *ndef = env->getGameDef()->ndef();
	bool is_position_valid;
	for (s16 z = ceilpos_min.Z; z <= ceilpos_max.Z; ++z) {
		for (s16 x = ceilpos_min.X; x <= ceilpos_max.X; ++x) {
			MapNode n = env->getMap().getNode(v3size(x, ceilpos_max.Y, z), &is_position_valid);
