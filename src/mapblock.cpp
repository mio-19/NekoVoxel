...
MapBlock::MapBlock(Map *parent, v3s32 pos, IGameDef *gamedef):
		m_parent(parent),
		m_pos(pos),
...
bool MapBlock::isValidPositionParent(v3s32 p)
{
...
MapNode MapBlock::getNodeParent(v3s32 p, bool *is_valid_position)
{
	if (!isValidPosition(p))
...
void MapBlock::copyTo(VoxelManipulator &dst)
{
	...
	dst.copyFrom(data, data_area, v3s32(0,0,0),
...
void MapBlock::copyFrom(VoxelManipulator &dst)
{
	...
	dst.copyTo(data, data_area, v3s32(0,0,0),
			getPosRelative(), data_size);
}
...
std::string analyze_block(MapBlock *block)
{
	...
	bool full_air = true;
	bool some_air = false;
	for(s32 z0=0; z0<MAP_BLOCKSIZE; z0++)
	for(s32 y0=0; y0<MAP_BLOCKSIZE; y0++)
	for(s32 x0=0; x0<MAP_BLOCKSIZE; x0++)
	{
		v3s32 p(x0,y0,z0);
