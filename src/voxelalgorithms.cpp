 ...
 */
const static v3s32 neighbor_dirs[6] = {
	v3s32(1, 0, 0), // right
	v3s32(0, 1, 0), // top
	v3s32(0, 0, 1), // back
	v3s32(0, 0, -1), // front
...
struct SunlightPropagationUnit{
	...
	SunlightPropagationUnit(v2s32 relpos, bool sunlit):
		relative_pos(relpos),
		is_sunlit(sunlit)
	{}
};
 ...
 */
bool is_sunlight_above(Map *map, v3s32 pos, const NodeDefManager *ndef)
{
	bool sunlight = true;
	mapblock_v3 source_block_pos;
	relative_v3 source_rel_pos;
	getNodeBlockPosWithOffset(pos + v3s32(0, 1, 0), source_block_pos,
		source_rel_pos);
		...
		for (auto it = oldnodes.cbegin(); it < oldnodes.cend(); ++it) {
			...
			if (f.light_propagates) {
				if (bank == LIGHTBANK_DAY && f.sunlight_propagates
					...
					for (const v3s32 &neighbor_dir : neighbor_dirs) {
						v3s32 p2 = p + neighbor_dir;
...
const VoxelArea block_borders[] = {
	VoxelArea(v3s32(15, 0, 0), v3s32(15, 15, 15)), //X+
	VoxelArea(v3s32(0, 15, 0), v3s32(15, 15, 15)), //Y+
	VoxelArea(v3s32(0, 0, 15), v3s32(15, 15, 15)), //Z+
	VoxelArea(v3s32(0, 0, 0), v3s32(15, 15, 0)),   //Z-
	VoxelArea(v3s32(0, 0, 0), v3s32(15, 0, 15)),   //Y-
	VoxelArea(v3s32(0, 0, 0), v3s32(0, 15, 15))    //X-
};
	...
	std::map<v3s32, MapBlock*> &modified_blocks)
{
	...
	for (LightBank bank : banks) {
		...
		for (direction d = 0; d < 6; d++) {
			// For each direction
			// Get neighbor block
			v3s32 otherpos = block->getPos() + neighbor_dirs[d];
 ...
 */
void fill_with_sunlight(MMVManip *vm, const NodeDefManager *ndef, v2s32 offset,
	bool light[MAP_BLOCKSIZE][MAP_BLOCKSIZE])
{
	...
	for (s32 x = 0; x < MAP_BLOCKSIZE; x++) {
		...
		v2s32 realpos = offset + v2s32(x, z);
...
{
	mapblock_v3 source_block_pos = pos + v3s32(0, 1, 0);
...
const VoxelArea block_pad[] = {
	VoxelArea(v3s32(15, 0, 0), v3s32(15, 15, 15)), //X+
	VoxelArea(v3s32(1, 15, 0), v3s32(14, 15, 15)), //Y+
	VoxelArea(v3s32(1, 1, 15), v3s32(14, 14, 15)), //Z+
	VoxelArea(v3s32(1, 1, 0), v3s32(14, 14, 0)),   //Z-
	VoxelArea(v3s32(1, 0, 0), v3s32(14, 0, 15)),   //Y-
	VoxelArea(v3s32(0, 0, 0), v3s32(0, 15, 15))    //X-
};
	...
	std::map<v3s32, MapBlock*> *modified_blocks)
{
	...
	// For each map block:
	for (s32 x = minblock.X; x <= maxblock.X; x++)
	for (s32 z = minblock.Z; z <= maxblock.Z; z++) {
		// Extract sunlight above.
		is_sunlight_above_block(map, v3s32(x, maxblock.Y, z), ndef, lights);
		v2s32 offset(x, z);
		offset *= MAP_BLOCKSIZE;
		// Reset the voxel manipulator.
		fill_with_sunlight(vm, ndef, offset, lights);
		// Copy sunlight data
		data.target_block = v3s32(x, minblock.Y - 1, z);
		for (s32 z = 0; z < MAP_BLOCKSIZE; z++)
		for (s32 x = 0; x < MAP_BLOCKSIZE; x++)
			data.data.emplace_back(v2s32(x, z), lights[z][x]);
	...
	for (blockpos.Z = minblock.Z; blockpos.Z <= maxblock.Z; blockpos.Z++) {
		...
		if (!block)
			// Skip not existing blocks.
			continue;
		v3s32 offset = block->getPosRelative();
	...
	bool light[MAP_BLOCKSIZE][MAP_BLOCKSIZE])
{
	...
	for (s32 x = 0; x < MAP_BLOCKSIZE; x++) {
		...
		bool lig = light[z][x];
		// For each node, downwards:
		for (s32 y = MAP_BLOCKSIZE - 1; y >= 0; y--) {
	...
	std::map<v3s32, MapBlock*> *modified_blocks)
{
	...
	// Copy sunlight data
	data.target_block = v3s32(blockpos.X, blockpos.Y - 1, blockpos.Z);
	for (s32 z = 0; z < MAP_BLOCKSIZE; z++)
	for (s32 x = 0; x < MAP_BLOCKSIZE; x++) {
		data.data.emplace_back(v2s32(x, z), lights[z][x]);
	}
...
s32 VoxelLineIterator::getIndex(v3s32 voxel){
	return
