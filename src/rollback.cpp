		...
		const std::list<ActionRow> & rows)
{
	...
	for (const ActionRow &row : rows) {
		...
		case RollbackAction::TYPE_SET_NODE:
			action.p            = v3size(row.x, row.y, row.z);
...
const std::list<ActionRow> RollbackManager::getRowsSince_range(
		time_t start_time, v3size p, int range, int limit)
{
...
float RollbackManager::getSuspectNearness(bool is_guess, v3size suspect_p,
...
std::string RollbackManager::getSuspect(v3size p, float nearness_shortcut,
		float min_nearness)
{
...
std::list<RollbackAction> RollbackManager::getNodeActors(v3size pos, int range,
