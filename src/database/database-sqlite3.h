...
public:
	...
	bool saveBlock(const v3size &pos, const std::string &data);
	void loadBlock(const v3size &pos, std::string *block);
	bool deleteBlock(const v3size &pos);
...
private:
	void bindPos(sqlite3_stmt *stmt, const v3size &pos, int index = 1);
