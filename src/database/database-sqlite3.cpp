...
inline void MapDatabaseSQLite3::bindPos(sqlite3_stmt *stmt, const v3s32 &pos, int index)
{
	SQLOK(sqlite3_bind_int64(stmt, index, getBlockAsInteger(pos)),
		"Internal error: failed to bind query at " __FILE__ ":" TOSTRING(__LINE__));
}

bool MapDatabaseSQLite3::deleteBlock(const v3s32 &pos)
{
	verifyDatabase();
...
bool MapDatabaseSQLite3::saveBlock(const v3s32 &pos, const std::string &data)
{
	verifyDatabase();
...
void MapDatabaseSQLite3::loadBlock(const v3s32 &pos, std::string *block)
{
	verifyDatabase();
...
void MapDatabaseSQLite3::listAllLoadableBlocks(std::vector<v3s32> &dst)
{
	verifyDatabase();
...
bool PlayerDatabaseSQLite3::loadPlayer(RemotePlayer *player, PlayerSAO *sao)
{
	...
	sao->setBasePosition(sqlite_to_v3d(m_stmt_player_load, 2));
