...
bool Database_Dummy::saveBlock(const v3size &pos, const std::string &data)
{
	m_database[getBlockAsInteger(pos)] = data;
	return true;
}

void Database_Dummy::loadBlock(const v3size &pos, std::string *block)
{
...
bool Database_Dummy::deleteBlock(const v3size &pos)
{
	m_database.erase(getBlockAsInteger(pos));
	return true;
}

void Database_Dummy::listAllLoadableBlocks(std::vector<v3size> &dst)
{
