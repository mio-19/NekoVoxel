...
v3size MapNode::getWallMountedDir(const NodeDefManager *nodemgr) const
{
	switch(getWallMounted(nodemgr))
	{
	case 0: default: return v3size(0,1,0);
	case 1: return v3size(0,-1,0);
	case 2: return v3size(1,0,0);
	case 3: return v3size(-1,0,0);
	case 4: return v3size(0,0,1);
	case 5: return v3size(0,0,-1);
	}
}
	...
	u8 neighbors = 0)
{
	...
	{
		v3size dir = n.getWallMountedDir(nodemgr);
...
static inline void getNeighborConnectingFace(
	const v3size &p, const NodeDefManager *nodedef,
...
u8 MapNode::getNeighbors(v3size p, Map *map) const
{
