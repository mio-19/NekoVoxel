...
v3s32 MapNode::getWallMountedDir(const NodeDefManager *nodemgr) const
{
	switch(getWallMounted(nodemgr))
	{
	case 0: default: return v3s32(0,1,0);
	case 1: return v3s32(0,-1,0);
	case 2: return v3s32(1,0,0);
	case 3: return v3s32(-1,0,0);
	case 4: return v3s32(0,0,1);
	case 5: return v3s32(0,0,-1);
	}
}
	...
	u8 neighbors = 0)
{
	...
	{
		v3s32 dir = n.getWallMountedDir(nodemgr);
...
static inline void getNeighborConnectingFace(
	const v3s32 &p, const NodeDefManager *nodedef,
...
u8 MapNode::getNeighbors(v3s32 p, Map *map) const
{
...
s8 MapNode::setLevel(const NodeDefManager *nodemgr, s32 level)
{
	s8 rest = 0;
	const ContentFeatures &f = nodemgr->get(*this);
	if (f.param_type_2 == CPT2_FLOWINGLIQUID
...
s8 MapNode::addLevel(const NodeDefManager *nodemgr, s32 add)
{
	s32 level = getLevel(nodemgr);
	level += add;
