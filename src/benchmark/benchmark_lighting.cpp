...
TEST_CASE("benchmark_lighting")
{
	...
	// Make a platform with a light below it.
	{
		...
		for (s32 z = -10; z <= 10; z++)
		for (s32 x = -10; x <= 10; x++)
			vm.setNodeNoEmerge(v3s32(x, 1, z), MapNode(content_wall));
		vm.setNodeNoEmerge(v3s32(0, -10, 0), MapNode(content_light));
	...
	BENCHMARK_ADVANCED("voxalgo::update_lighting_nodes")(Catch::Benchmark::Chronometer meter) {
		std::map<v3s32, MapBlock*> modified_blocks;
		meter.measure([&] {
			map.addNodeAndUpdate(v3s32(0, 0, 0), MapNode(content_light), modified_blocks);
			map.removeNodeAndUpdate(v3s32(0, 0, 0), modified_blocks);
		});
	};
	...
	BENCHMARK_ADVANCED("voxalgo::blit_back_with_light")(Catch::Benchmark::Chronometer meter) {
		std::map<v3s32, MapBlock*> modified_blocks;
		MMVManip vm(&map);
		vm.initialEmerge(v3s32(0, 0, 0), v3s32(0, 0, 0), false);
		meter.measure([&] {
			vm.setNodeNoEmerge(v3s32(0, 0, 0), MapNode(content_light));
			voxalgo::blit_back_with_light(&map, &vm, &modified_blocks);
			vm.setNodeNoEmerge(v3s32(0, 0, 0), MapNode(CONTENT_AIR));
			voxalgo::blit_back_with_light(&map, &vm, &modified_blocks);
		});
	};
}
