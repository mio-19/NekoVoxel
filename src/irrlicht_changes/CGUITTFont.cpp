...
core::array<scene::ISceneNode*> CGUITTFont::addTextSceneNode(const wchar_t* text, scene::ISceneManager* smgr, scene::ISceneNode* parent, const video::SColor& color, bool center)
{
	...
	**/
	...
	while (*text)
	{
		...
		else
		{
			...
			if (n > 0)
			{
				...
				mani->scale(meshcopy, vector3df((f64)letter_size.Width, (f64)letter_size.Height, 1));
	...
	for (u32 i = 0; i < glyph_indices.size(); ++i)
	{
		...
		f64 page_texture_size = (f64)current_tex->getSize().Width;
		...
		//  0---1
		//
		f64 u1 = glyph.source_rect.UpperLeftCorner.X / page_texture_size;
		f64 u2 = u1 + (glyph.source_rect.getWidth() / page_texture_size);
		f64 v1 = glyph.source_rect.UpperLeftCorner.Y / page_texture_size;
		f64 v2 = v1 + (glyph.source_rect.getHeight() / page_texture_size);
		...
		//pv[0].TCoords.Y = pv[1].TCoords.Y = (letter_size.Height - 1) / static_cast<f64>(letter_size.Height);
		//pv[1].TCoords.X = pv[3].TCoords.X = (letter_size.Width - 1)  / static_cast<f64>(letter_size.Width);
