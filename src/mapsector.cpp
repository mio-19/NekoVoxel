...
MapSector::MapSector(Map *parent, v2s32 pos, IGameDef *gamedef):
		m_parent(parent),
		m_pos(pos),
		m_gamedef(gamedef)
{
...
MapBlock * MapSector::getBlockBuffered(s32 y)
{
	MapBlock *block;
	...
	std::unordered_map<s32, MapBlock*>::const_iterator n = m_blocks.find(y);
...
MapBlock * MapSector::getBlockNoCreateNoEx(s32 y)
{
	return getBlockBuffered(y);
}

MapBlock * MapSector::createBlankBlockNoInsert(s32 y)
{
	assert(getBlockBuffered(y) == NULL);	// Pre-condition

	v3s32 blockpos_map(m_pos.X, y, m_pos.Y);
...
MapBlock * MapSector::createBlankBlock(s32 y)
{
...
void MapSector::insertBlock(MapBlock *block)
{
	...
	v2s32 p2d(block->getPos().X, block->getPos().Z);
	assert(p2d == m_pos);
