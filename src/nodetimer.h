...
public:
	NodeTimer() = default;
	NodeTimer(const v3size &position_):
		position(position_) {}
	NodeTimer(f32 timeout_, f32 elapsed_, v3size position_):
...
public:
	...
	// Get timer
	NodeTimer get(const v3size &p) {
		std::map<v3size, std::multimap<double, NodeTimer>::iterator>::iterator n =
			m_iterators.find(p);
...
private:
	...
	std::map<v3size, std::multimap<double, NodeTimer>::iterator> m_iterators;
