...
public:
	...
	// Get timer
	NodeTimer get(const v3s32 &p) {
		std::map<v3s32, std::multimap<double, NodeTimer>::iterator>::iterator n =
			m_iterators.find(p);
...
private:
	...
	std::map<v3s32, std::multimap<double, NodeTimer>::iterator> m_iterators;
