	...
	*/
	{
		...
		std::unordered_set<v3s32> node_meta_updates;
		...
		while (!m_unsent_map_edit_queue.empty()) {
			...
			case MEET_OTHER:
				...
				for (const v3s32 &modified_block : event->modified_blocks) {
			...
			*/
			if (!far_players.empty()) {
				...
				std::map<v3s32, MapBlock*> modified_blocks2;
				for (const v3s32 &modified_block : event->modified_blocks) {
	...
	const ParticleParameters &p)
{
	static thread_local const float radius =
			g_settings->getS32("max_block_send_distance") * MAP_BLOCKSIZE * BS;
...
void Server::SendTimeOfDay(session_t peer_id, u16 time, f64 time_speed)
{
...
void Server::SendMovePlayer(session_t peer_id)
{
	...
	NetworkPacket pkt(TOCLIENT_MOVE_PLAYER, sizeof(v3d) + sizeof(f64) * 2, peer_id);
...
void Server::SendActiveObjectRemoveAdd(RemoteClient *client, PlayerSAO *playersao)
{
	...
	static thread_local const s32 radius =
		g_settings->getS32("active_object_send_range_blocks") * MAP_BLOCKSIZE;
	...
	static thread_local const s32 player_transfer_dist =
		g_settings->getS32("player_transfer_distance") * MAP_BLOCKSIZE;

	s32 player_radius = player_transfer_dist == 0 && is_transfer_limited ?
		radius : player_transfer_dist;

	s32 my_radius = MYMIN(radius, playersao->getWantedRange() * MAP_BLOCKSIZE);
	if (my_radius <= 0)
		my_radius = radius;
...
void Server::sendMetadataChanged(const std::unordered_set<v3s32> &positions, float far_d_nodes)
{
	...
	for (session_t i : clients) {
		...
		for (const v3s32 pos : positions) {
			NodeMetadata *meta = m_env->getMap().getNodeMetadata(pos);

			if (!meta)
				continue;

			v3s32 block_pos = getNodeBlockPos(pos);
...
{
	thread_local const int net_compression_level = rangelim(g_settings->getS32("map_compression_level_net"), -1, 9);
...
bool Server::SendBlock(session_t peer_id, const v3s32 &blockpos)
{
...
void Server::setLocalPlayerAnimations(RemotePlayer *player,
		v2s32 animation_frames[4], f64 frame_speed)
{
	sanity_check(player);
...
v3d Server::findSpawnPos()
{
	...
	for (s32 i = 0; i < 4000 && !is_good; i++) {
		...
		v2s32 nodepos2d = v2s32(
			-range + myrand_range(0, range*2),
			-range + myrand_range(0, range*2));
		// Get spawn level at point
		s32 spawn_level = m_emerge->getSpawnLevelAtPoint(nodepos2d);
		...
		v3s32 nodepos(nodepos2d.X, spawn_level, nodepos2d.Y);
