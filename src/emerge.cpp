...
public:
	...
	bool pushBlock(const v3size &pos);
...
protected:
	...
	void runCompletionCallbacks(
		const v3size &pos, EmergeAction action,
...
private:
	...
	Event m_queue_event;
	std::queue<v3size> m_block_queue;

	bool popBlockEmerge(v3size *pos, BlockEmergeData *bedata);

	EmergeAction getBlockOrStartGen(
		const v3size &pos, bool allow_gen, MapBlock **block, BlockMakeData *data);
	MapBlock *finishGen(v3size pos, BlockMakeData *bmdata,
...
void EmergeManager::initMapgens(MapgenParams *params)
{
	...
	v3size csize = v3size(1, 1, 1) * (params->chunksize * MAP_BLOCKSIZE);
...
bool EmergeManager::isBlockInQueue(v3size pos)
{
	MutexAutoLock queuelock(m_queue_mutex);
...
v3size EmergeManager::getContainingChunk(v3size blockpos, s16 chunksize)
{
	s16 coff = -chunksize / 2;
	v3size chunk_offset(coff, coff, coff);
...
int EmergeManager::getSpawnLevelAtPoint(v2size p)
{
...
// TODO(hmmmm): Move this to ServerMap
bool EmergeManager::isBlockUnderground(v3size blockpos)
{
	...
	bool *entry_already_exists)
{
	...
	std::pair<std::map<v3size, BlockEmergeData>::iterator, bool> findres;
...
bool EmergeManager::popBlockEmergeData(v3size pos, BlockEmergeData *bedata)
{
...
bool EmergeThread::pushBlock(const v3size &pos)
{
...
void EmergeThread::runCompletionCallbacks(const v3size &pos, EmergeAction action,
	const EmergeCallbackList &callbacks)
{
...
bool EmergeThread::popBlockEmerge(v3size *pos, BlockEmergeData *bedata)
{
	MutexAutoLock queuelock(m_emerge->m_queue_mutex);
...
EmergeAction EmergeThread::getBlockOrStartGen(
	const v3size &pos, bool allow_gen, MapBlock **block, BlockMakeData *bmdata)
{
...
MapBlock *EmergeThread::finishGen(v3size pos, BlockMakeData *bmdata,
	std::map<v3size, MapBlock *> *modified_blocks)
{
	MutexAutoLock envlock(m_server->m_env_mutex);
	...
	*/
	...
	v3size minp = bmdata->blockpos_min * MAP_BLOCKSIZE;
	v3size maxp = bmdata->blockpos_max * MAP_BLOCKSIZE +
				 v3size(1,1,1) * (MAP_BLOCKSIZE - 1);
