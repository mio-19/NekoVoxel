...
std::unordered_map<u16, std::vector<v3size>> FacePositionCache::cache;
std::mutex FacePositionCache::cache_mutex;

// Calculate the borders of a "d-radius" cube
const std::vector<v3size> &FacePositionCache::getFacePositions(u16 d)
{
	MutexAutoLock lock(cache_mutex);
	std::unordered_map<u16, std::vector<v3size>>::const_iterator it = cache.find(d);
	if (it != cache.end())
		return it->second;
...
const std::vector<v3size> &FacePositionCache::generateFacePosition(u16 d)
{
	cache[d] = std::vector<v3size>();
	std::vector<v3size> &c = cache[d];
	if (d == 0) {
