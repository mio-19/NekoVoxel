...
{
	PrioritySortedBlockTransfer(float a_priority, const v3size &a_pos, session_t a_peer_id)
	{
		priority = a_priority;
		pos = a_pos;
		peer_id = a_peer_id;
...
public:
	...
	*/
	...
	void GotBlock(v3size p);

	void SentBlock(v3size p);

	void SetBlockNotSent(v3size p);
	void SetBlocksNotSent(std::map<v3size, MapBlock*> &blocks);
	 ...
	 */
	void ResendBlockIfOnWire(v3size p);

	u32 getSendingCount() const { return m_blocks_sending.size(); }

	bool isBlockSent(v3size p) const
	{
...
private:
	...
	*/
	std::set<v3size> m_blocks_sent;
	s16 m_nearest_unsent_d = 0;
	v3size m_last_center;
	...
	*/
	std::map<v3size, float> m_blocks_sending;
	...
	/*
		...
		List of block positions.
	*/
	std::set<v3size> m_blocks_modified;
...
public:
	...
	void markBlockposAsNotSent(const v3size &pos);
